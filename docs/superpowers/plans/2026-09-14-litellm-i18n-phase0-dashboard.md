...
Important scope limit: Phase 0 does not localize any unconditionally-visible element, because the only localized UI is inside the account menu popover, whose contents are not part of the prerendered HTML. So an HTML text grep for a Chinese string is not a valid check here. The no-flash claim rests on the inline `resources` forcing i18next's synchronous load path, on Task 2's test asserting that `t("logout")` returns the Chinese string synchronously at import, and on the exported document declaring `lang="zh"`. `initAsync: false` is deliberate belt-and-braces rather than the load-bearing mechanism: a Task 8 probe flipped it to `true` and the whole i18n suite still passed, so no test discriminates it, and the synchronous `t()` assertion is what would fail if resources ever moved behind an async backend. Localizing the login page heading should be the first item of Phase 1 precisely so a HTML text grep becomes possible.
