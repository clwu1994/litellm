          ...
          <div className="flex flex-wrap items-center gap-1">
            {displayTags.map((tag, index) => (
              <SimpleTooltip
                ...
                content={
                  <div>
                    <div>
                      <span className="text-muted-foreground">{t("topKeys.tagTooltip.name")}</span> {tag.tag}
                    </div>
                    <div>
                      <span className="text-muted-foreground">{t("topKeys.tagTooltip.spend")}</span>{" "}
            ...
            {hasMoreTags && (
              <button
                ...
                title={isExpanded ? t("topKeys.showFewerTags") : t("topKeys.showAllTags")}
              >
                {isExpanded ? (
  ...
  return (
    <>
      <div className="mb-4 flex justify-between items-center">
        <RadioGroup
          aria-label={t("topKeys.limitAria")}
                ...
                <div className="relative z-floating p-3 bg-black/90 shadow-lg rounded-lg text-white max-w-xs">
                  <div className="space-y-1.5">
                    <div className="text-sm">
                      <span className="text-muted-foreground">{t("topKeys.chartTooltip.keyAlias")}</span>{" "}
                      <span className="font-mono text-gray-100 break-all">{item?.key_alias}</span>
                    </div>
                    <div className="text-sm">
                      <span className="text-muted-foreground">{t("topKeys.chartTooltip.keyId")}</span>{" "}
                      <span className="font-mono text-gray-100 break-all">{item?.api_key}</span>
                    </div>
                    <div className="text-sm">
                      <span className="text-muted-foreground">{t("topKeys.chartTooltip.spend")}</span>{" "}
