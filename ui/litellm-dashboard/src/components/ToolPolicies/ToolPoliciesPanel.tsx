...
function getTrendSubtitle(newToday: number, newYesterday: number, t: TFunction<"policies">): string | undefined {
  const diff = newToday - newYesterday;
  if (diff === 0) return undefined;
  return diff > 0 ? t("toolPolicies.metrics.trendUp", { diff }) : t("toolPolicies.metrics.trendDown", { diff });
}
  ...
  const handleInputPolicyChange = useCallback(
    async (toolName: string, newPolicy: string) => {
      ...
      try {
        ...
        toast.fromError(t("toolPolicies.updateInputFailed", { error: toMessage(e, t("toolPolicies.unknownError")) }));
      } finally {
  ...
  const handleOutputPolicyChange = useCallback(
    async (toolName: string, newPolicy: string) => {
      ...
      try {
        ...
        toast.fromError(t("toolPolicies.updateOutputFailed", { error: toMessage(e, t("toolPolicies.unknownError")) }));
      } finally {
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        />
        <MetricCard label={t("toolPolicies.metrics.totalTools")} value={totalTools} />
        <MetricCard
          label={t("toolPolicies.metrics.blockedTools")}
          value={blockedCount}
      ...
      {needsReviewTools.length > 0 && (
        <div className="bg-warning/10 border border-warning/20 rounded-lg p-4 mb-6">
          <h2 className="text-sm font-semibold text-warning mb-1">{t("toolPolicies.needsReview.title")}</h2>
          <p className="text-sm text-warning mb-3">
            {needsReviewTools.length === 1
              ? t("toolPolicies.needsReview.singular", { tools: needsReviewTools.length })
              : t("toolPolicies.needsReview.plural", { tools: needsReviewTools.length })}
          </p>
          <div className="flex flex-wrap gap-2">
            {needsReviewTools.map((tool) => (
              ...
              >
                ...
                >
                  {t("toolPolicies.needsReview.review")}
                </button>
              </span>
            ))}
          </div>
      ...
      {query.isError && (
        <div
          ...
          role="alert"
        >
          {toMessage(query.error, t("toolPolicies.loadFailed"))}
        </div>
      )}
