...
export default function CredentialsPanel() {
  ...
  const handleUpdateCredential = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      toast.success(t("credentials.toastUpdated"));
      setIsUpdateModalOpen(false);
      await refetchCredentials();
    } catch (error) {
      toast.error(t("credentials.toastUpdateFailed"));
    }
  };
  ...
  const handleAddCredential = async (values: Record<string, unknown>) => {
    ...
    try {
      ...
      toast.success(t("credentials.toastAdded"));
      setIsAddModalOpen(false);
      await refetchCredentials();
    } catch (error) {
      toast.error(t("credentials.toastAddFailed"));
    }
  };
  ...
  const handleDeleteCredential = async () => {
    ...
    try {
      ...
      toast.success(t("credentials.toastDeleted"));
      await refetchCredentials();
    } catch (error) {
      toast.error(t("credentials.toastDeleteFailed"));
    } finally {
  ...
  return (
    <div className="mx-auto flex w-full flex-auto flex-col gap-4 overflow-y-auto p-2">
      ...
      <DeleteResourceModal
        ...
        resourceInformation={[
          { label: t("credentials.infoName"), value: credentialToDelete?.credential_name },
          {
            label: t("credentials.infoProvider"),
