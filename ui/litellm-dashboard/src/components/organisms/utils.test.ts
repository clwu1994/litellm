...
const t = i18n.getFixedT("en", "templates");
...
describe("simplifyKeyGenerateError", () => {
  ...
  describe("when error is NOT related to /key/generate", () => {
    it("should return the original error message for generic errors", () => {
      ...
      const result = simplifyKeyGenerateError(error, t);
  ...
  describe("Chinese copy", () => {
    const tZh = i18n.getFixedT("zh", "templates");

    it("returns the Chinese permission message and hides the English original", () => {
      const error = "Team member does not have permissions for endpoint: KeyManagementRoutes.KEY_GENERATE.";
      const result = simplifyKeyGenerateError(error, tZh);

      expect(result).toBe("团队成员没有为该团队生成密钥的权限。请联系 Proxy 管理员配置团队成员权限设置。");
      expect(result).not.toContain("Team member does not have permission");
    });

    it("returns the Chinese generic wrapper with the raw error and hides the English original", () => {
      const result = simplifyKeyGenerateError("boom", tZh);

      expect(result).toBe("创建密钥时出错：boom");
      expect(result).not.toContain("Error creating the key");
    });

    it("keeps null and undefined byte-identical inside the Chinese wrapper", () => {
      expect(simplifyKeyGenerateError(null, tZh)).toBe("创建密钥时出错：null");
      expect(simplifyKeyGenerateError(undefined, tZh)).toBe("创建密钥时出错：undefined");
    });
  });
});
