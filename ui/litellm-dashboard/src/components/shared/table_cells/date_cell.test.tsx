...
describe("formatCellDate", () => {
  it("formats datetime precision as 'MMM D, HH:mm:ss' without a year", () => {
    expect(formatCellDate(new Date(2026, 6, 7, 9, 50, 13), "datetime", t)).toBe("Jul 7, 09:50:13");
  });

  it("zero-pads hours, minutes and seconds", () => {
    expect(formatCellDate(new Date(2026, 0, 2, 1, 2, 3), "datetime", t)).toBe("Jan 2, 01:02:03");
  });

  it("formats date precision as 'MMM D, YYYY' with no time", () => {
    expect(formatCellDate(new Date(2026, 11, 31, 23, 59, 59), "date", t)).toBe("Dec 31, 2026");
  });
});
...
describe("formatFullTimestamp", () => {
  it("includes year, 24h time and the IANA timezone", () => {
    ...
    expect(formatFullTimestamp(new Date(2026, 6, 7, 9, 50, 13), t)).toBe(`Jul 7, 2026, 09:50:13 (${timeZone})`);
  });
});
