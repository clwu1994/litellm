...
interface RelativeTimeOption {
  labelKey:
    | "advancedDatePicker.today"
    | "advancedDatePicker.last7Days"
    | "advancedDatePicker.last30Days"
    | "advancedDatePicker.monthToDate"
    | "advancedDatePicker.yearToDate";
  shortLabel: string;
...
const relativeTimeOptions: RelativeTimeOption[] = [
  {
    labelKey: "advancedDatePicker.today",
    shortLabel: "today",
    getValue: () => ({
  ...
  {
    labelKey: "advancedDatePicker.last7Days",
    shortLabel: "7d",
    getValue: () => ({
  ...
  {
    labelKey: "advancedDatePicker.last30Days",
    shortLabel: "30d",
    getValue: () => ({
  ...
  {
    labelKey: "advancedDatePicker.monthToDate",
    shortLabel: "MTD",
    getValue: () => ({
  ...
  {
    labelKey: "advancedDatePicker.yearToDate",
    shortLabel: "YTD",
    getValue: () => ({
...
const AdvancedDatePicker: React.FC<AdvancedDatePickerProps> = ({
  ...
  const resolvedLabel = label ?? t("advancedDatePicker.label");
  ...
  const validateDateRange = useCallback(() => {
    ...
    if (!start.isValid() || !end.isValid()) {
      return { isValid: false, error: t("advancedDatePicker.invalidDateFormat") };
    }

    if (end.isBefore(start)) {
      return { isValid: false, error: t("advancedDatePicker.endDateBeforeStart") };
    }
  ...
  const formatDisplayRange = useCallback(
    (from: Date | undefined, to: Date | undefined) => {
      if (!from || !to) return t("advancedDatePicker.selectDateRange");
            ...
            <div className="flex">
              ...
              <div className="w-1/2 border-r border-border">
                <div className="p-3 border-b border-border">
                  <span className="text-sm font-semibold text-foreground">{t("advancedDatePicker.relativeTime")}</span>
                </div>
              ...
              <div className="w-1/2 relative">
                <div className="p-3.5 border-b border-border">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-foreground">
                      {t("advancedDatePicker.startAndEndDates")}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-6 pb-20">
                  {/* Start date */}
                  <div>
                    <label className="text-sm text-foreground mb-1 block">{t("advancedDatePicker.startDate")}</label>
                    <input
                      type="date"
                      value={startDate}
                  ...
                  {/* End date */}
                  <div>
                    <label className="text-sm text-foreground mb-1 block">{t("advancedDatePicker.endDate")}</label>
                    <input
                      type="date"
                      value={endDate}
                  ...
                  {tempValue.from && tempValue.to && validation.isValid && (
                    <div className="bg-info/10 p-3 rounded-md space-y-1">
                      <div className="text-xs text-info">
                        <span className="font-medium">{t("advancedDatePicker.from")}</span>{" "}
                        {moment(tempValue.from).format("MMM D, YYYY [at] HH:mm:ss")}
                      </div>
                      <div className="text-xs text-info">
                        <span className="font-medium">{t("advancedDatePicker.to")}</span>{" "}
