...
const UsefulLinksManagement: React.FC<UsefulLinksManagementProps> = ({ accessToken, userRole }) => {
  ...
  const handleAddLink = async () => {
    ...
    if (await saveLinksToBackend(updatedLinks)) {
      ...
      toast.success(t("usefulLinks.added"));
    }
  };
  ...
  const handleUpdateLink = async () => {
    ...
    if (await saveLinksToBackend(updatedLinks)) {
      setLinks(updatedLinks);
      setEditingLink(null);
      toast.success(t("usefulLinks.updated"));
    }
  };
  ...
  const deleteLink = async (linkId: string) => {
    ...
    if (await saveLinksToBackend(updatedLinks)) {
      setLinks(updatedLinks);
      toast.success(t("usefulLinks.deleted"));
    }
  };
  ...
  const handleSaveRearranging = async () => {
    if (await saveLinksToBackend(links)) {
      setIsRearranging(false);
      setOriginalLinksOrder([]);
      toast.success(t("usefulLinks.orderSaved"));
    }
  };
  ...
  return (
    <Card className="mb-6 px-6">
      ...
      {isExpanded && (
        <div className="mt-4">
          <div className="mb-6">
            <p className="text-sm font-medium text-foreground mb-2">{t("usefulLinks.addNewLink")}</p>
              ...
              <Table className="[&_td]:py-0.5 [&_th]:py-1">
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="py-1 h-8">{t("shared.actions")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                            ...
                            {isRearranging ? (
                              <div className="flex space-x-2">
                                <TableIconActionButton
                                  ...
                                  disabled={index === 0}
                                  disabledTooltipText={t("usefulLinks.atTop")}
                                ...
                                <TableIconActionButton
                                  ...
                                  disabledTooltipText={t("usefulLinks.atBottom")}
