...
import { formatExpiryDate, getDaysUntilExpiration } from "@/utils/licenseUtils";
...
const UsageMeter = ({ label, used, total }: MeterData) => {
  ...
  return (
    <Meter
      value={used}
      max={total}
      aria-valuetext={t("usageCard.aria", { used: used.toLocaleString(), total: total.toLocaleString() })}
    >
...
const buildMeters = (data: RemainingUsage | null, t: TFunction<"common">): MeterData[] => {
  if (!data) return [];
  return [
    ...(data.total_users != null
      ? [{ label: t("usageCard.seats"), used: data.total_users_used, total: data.total_users }]
      : []),
    ...(data.total_teams != null
      ? [{ label: t("usageCard.teams"), used: data.total_teams_used, total: data.total_teams }]
      : []),
  ];
};

const expirationStatus = (expirationDate: string, t: TFunction<"common">): string => {
  const days = getDaysUntilExpiration(expirationDate);
  if (days === null) {
    return t("usageCard.noExpiration");
  }
  return days < 0
    ? t("usageCard.expired", { date: formatExpiryDate(expirationDate) })
    : t("usageCard.expires", { date: formatExpiryDate(expirationDate) });
};
...
export default function SidebarUsageCard({ accessToken, collapsed, onExpandRail }: SidebarUsageCardProps) {
  ...
  const subtitle = licenseInfo?.expiration_date
    ? expirationStatus(licenseInfo.expiration_date, t)
    : t("usageCard.activePlan");
  const meters = buildMeters(data, t);
    ...
    <Collapsible defaultOpen className="overflow-hidden rounded-xl border border-sidebar-border bg-sidebar">
      <CollapsibleTrigger className="group/usage flex w-full items-center gap-2.5 px-3 py-2.5 text-left transition-colors hover:bg-sidebar-accent">
        ...
        <span className="min-w-0 flex-1 leading-tight">
          <span className="block text-[13px] font-semibold text-foreground">{t("usageCard.title")}</span>
      ...
      <CollapsibleContent className="flex flex-col gap-3 px-3 pt-0.5 pb-3">
        {isLoading && meters.length === 0 ? (
          <div className="flex items-center gap-2 py-1 text-xs text-muted-foreground">
            <Loader2 className="size-3.5 animate-spin" /> {t("usageCard.loading")}
          </div>
        ) : (
