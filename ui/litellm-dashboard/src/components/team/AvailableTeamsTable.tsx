...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("availableTeams.empty.title")}</div>
      <div className="text-sm text-muted-foreground">
        <Trans
          ns="teams"
          i18nKey="availableTeams.empty.hint"
          components={{
            docsLink: (
              <a
...
const AvailableTeamsTable: React.FC<AvailableTeamsTableProps> = ({ teams, isLoading, onJoinTeam }) => {
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("availableTeams.loading")}
