...
export const BlogDropdown: React.FC = () => {
  ...
  const renderMenuContent = () => {
    if (isLoading) {
      return (
        <div className="flex items-center px-2 py-1.5 text-sm">
          <LoaderCircle role="img" aria-label={t("topbar.blogLoading")} className="size-4 animate-spin" />
        </div>
      );
    }
    ...
    if (isError) {
      return (
        <div className="flex items-center gap-2 px-2 py-1.5 text-sm">
          <span className="text-destructive">{t("topbar.blogError")}</span>
    ...
    if (!data || data.posts.length === 0) {
      return <div className="px-2 py-1.5 text-sm text-muted-foreground">{t("topbar.blogEmpty")}</div>;
    }
