...
const COPY = {
  custom: {
    overriddenKey: "autoRouterConfig.classifier.opening.customOverridden",
    defaultKey: "autoRouterConfig.classifier.opening.customDefault",
    explainerKey: "autoRouterConfig.classifier.opening.customExplainer",
    placeholder: CUSTOM_PLACEHOLDER,
  },
  builtIn: {
    overriddenKey: "autoRouterConfig.classifier.opening.builtInOverridden",
    defaultKey: "autoRouterConfig.classifier.opening.builtInDefault",
    explainerKey: "autoRouterConfig.classifier.opening.builtInExplainer",
    ...
    <div>
      {rubricSummary && (
        <p className="mb-1 text-xs text-muted-foreground">
          {isOverridden
            ? t("autoRouterConfig.classifier.opening.customOnRubric", { rubric: t(rubricSummary.labelKey) })
            : t("autoRouterConfig.classifier.opening.rubricOnly", { rubric: t(rubricSummary.labelKey) })}
        </p>
      )}
      <div className="flex items-center gap-2">
        <Button type="button" size="sm" variant="outline" onClick={openEditor}>
          {isOverridden
            ? t("autoRouterConfig.classifier.opening.editCustom")
            : t("autoRouterConfig.classifier.opening.customize")}
        </Button>
        {isOverridden && (
          <Button
            type="button"
          ...
          >
            {t("autoRouterConfig.classifier.opening.reset")}
          </Button>
        )}
      </div>
      <p className="mt-1 text-xs text-muted-foreground">{t(isOverridden ? copy.overriddenKey : copy.defaultKey)}</p>

      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle>{t("autoRouterConfig.classifier.opening.dialogTitle")}</DialogTitle>
          </DialogHeader>

          {tierSource.kind === "builtIn" && (
            <div>
              <label className="text-sm font-medium" htmlFor="base-classification-rubric">
                {t("autoRouterConfig.classifier.opening.baseRubric")}
              </label>
              <Select
                items={Object.entries(CLASSIFICATION_RUBRIC_DESCRIPTIONS).map(([rubric, description]) => ({
                  value: rubric,
                  label: t(description.labelKey),
                }))}
              ...
              >
                <SelectTrigger
                  ...
                  aria-label={t("autoRouterConfig.classifier.opening.baseRubricAria")}
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {tierSource.rubricRestriction ??
                  (draftRubricSummary ? t(draftRubricSummary.descriptionKey) : undefined)}
              </p>
            </div>
          )}

          <p className="text-sm text-muted-foreground">{t(copy.explainerKey)}</p>

          <div className="mt-3 space-y-4">
            <div>
              <label className="text-sm font-medium" htmlFor="classification-instructions">
                {t("autoRouterConfig.classifier.opening.instructions")}
              </label>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("autoRouterConfig.classifier.opening.instructionsHelp")}
              </p>
              <Textarea
                ...
                aria-label={t("autoRouterConfig.classifier.opening.instructionsAria")}
            ...
            <div>
              <label className="text-sm font-medium" htmlFor="calibration-examples">
                {t("autoRouterConfig.classifier.opening.examples")}
              </label>
              <p className="mt-1 text-xs text-muted-foreground">
                {t("autoRouterConfig.classifier.opening.examplesHelp")}
              </p>
              <Textarea
                ...
                aria-label={t("autoRouterConfig.classifier.opening.examplesAria")}
                className="mt-2 font-mono text-xs"
              />
            </div>
          </div>
          ...
          <div className="mt-3">
            <p className="text-xs font-medium">{t("autoRouterConfig.classifier.opening.sends")}</p>
            {preview.status === "loading" && (
              <p className="mt-1 text-xs text-muted-foreground">{t("autoRouterConfig.classifier.opening.loading")}</p>
            )}
            {preview.status === "error" && (
              <p className="mt-1 text-xs text-muted-foreground">{t("autoRouterConfig.classifier.opening.loadError")}</p>
            )}
            {preview.status === "ready" && (
              <pre
                aria-label={t("autoRouterConfig.classifier.opening.previewAria")}
          ...
          <DialogFooter className="mt-4">
            <Button type="button" variant="outline" onClick={() => setIsOpen(false)}>
              {t("autoRouterConfig.classifier.opening.cancel")}
            </Button>
            <Button type="button" onClick={handleSave}>
              {t("autoRouterConfig.classifier.opening.save")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
