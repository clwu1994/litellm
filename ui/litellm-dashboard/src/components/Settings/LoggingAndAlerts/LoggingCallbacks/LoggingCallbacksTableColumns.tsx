...
const callbackModeLabel = (mode: string, t: TFunction<"settings">): string => {
  if (mode === "success") return t("loggingCallbacks.modeSuccess");
  if (mode === "failure") return t("loggingCallbacks.modeFailure");
  if (mode === "success_and_failure") return t("loggingCallbacks.modeSuccessAndFailure");
  return mode;
};
...
function CallbackRowActions({ callback, onTest, onEdit, onDelete }: CallbackRowActionsProps) {
  ...
  if (callback.read_only) {
    return (
      <span className="text-xs text-muted-foreground" title={t("loggingCallbacks.readOnlyTitle")}>
        {t("loggingCallbacks.readOnly")}
      </span>
    );
  }
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        aria-label={t("loggingCallbacks.openActionsAria")}
...
export const getLoggingCallbacksTableColumns = ({
...
}: LoggingCallbacksTableColumnsDeps): ColumnDef<CallbackRow>[] => [
  {
    id: "name",
    accessorKey: "name",
    meta: { title: t("loggingCallbacks.callbackName") },
    header: t("loggingCallbacks.callbackName"),
    enableSorting: false,
    cell: ({ row }) => {
  ...
  {
    id: "mode",
    meta: { title: t("loggingCallbacks.mode"), skeleton: "badge" },
    header: t("loggingCallbacks.mode"),
    size: 240,
    enableSorting: false,
    cell: ({ row }) => {
