...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("loggingCallbacks.emptyTitle")}</div>
      <div className="text-sm text-muted-foreground">{t("loggingCallbacks.emptyDescription")}</div>
    </div>
  );
}
...
export const LoggingCallbacksTable: React.FC<LoggingCallbacksProps> = ({
  ...
  return (
    <div className="mt-4 flex w-full flex-col gap-4">
      <h3 className="text-lg font-semibold tracking-tight text-foreground">{t("loggingCallbacks.activeTitle")}</h3>
      <div>
        <Button onClick={onAdd}>
          <Plus />
          {t("loggingCallbacks.addButton")}
        </Button>
      </div>
      <DataTable
        ...
        isLoading={isLoading}
        loadingMessage={t("loggingCallbacks.loading")}
