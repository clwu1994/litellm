...
import { defaultRoleDisplayNameKeys } from "./constants";
...
export default function RoleMappings({ roleMappings }: { roleMappings: RoleMappingsType | undefined }) {
  ...
  const roleDisplayName = (role: string) => {
    const key = defaultRoleDisplayNameKeys[role];
  ...
  const roleMappingsColumns: ColumnDef<RoleMappingRow>[] = [
    {
      id: "role",
      accessorKey: "role",
      header: t("sso.roleMappings.role"),
      cell: ({ row }) => <strong className="font-semibold">{roleDisplayName(row.original.role)}</strong>,
    },
    {
      id: "groups",
      accessorKey: "groups",
      header: t("sso.roleMappings.mappedGroups"),
      cell: ({ row }) =>
        row.original.groups.length > 0 ? (
          ...
          <span className="text-muted-foreground italic">{t("sso.roleMappings.none")}</span>
        ),
    },
  ];
  return (
    <Card>
      <CardContent>
        <div className="flex items-center gap-3">
          ...
          <h3 className="mb-2 text-2xl font-semibold text-foreground">{t("sso.roleMappings.title")}</h3>
        </div>
        <div className="space-y-8">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h5 className="mb-2 text-base font-semibold text-foreground">{t("sso.form.groupClaim")}</h5>
              <div>
                <code className={inlineCodeClass}>{roleMappings.group_claim}</code>
              </div>
            </div>
            <div>
              <h5 className="mb-2 text-base font-semibold text-foreground">{t("sso.form.defaultRole")}</h5>
              <div>
                <strong className="font-semibold">{roleDisplayName(roleMappings.default_role)}</strong>
              </div>
            </div>
          </div>
