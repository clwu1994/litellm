...
export default function AddFallbacks({ accessToken, value = [], onChange }: AddFallbacksProps) {
  ...
  const handleSaveAll = async () => {
    ...
    if (invalidGroups.length > 0) {
      toast.error(t("fallbacks.incompleteGroups", { count: invalidGroups.length }));
      return;
    }
    ...
    if (onChange) {
      ...
      try {
        ...
        toast.success(t("fallbacks.addedSuccess", { count: groups.length }));
        handleCancel();
      } catch (error) {
      ...
      toast.fromError(t("fallbacks.onChangeMissing"));
    }
  };
        ...
        {groups.length > 0 && (
          <div className="flex items-center justify-end space-x-3 pt-6 mt-6 border-t border-border">
            ...
            <Button variant="outline" onClick={handleSaveAll} disabled={groups.length === 0 || isSaving}>
              ...
              {isSaving ? t("fallbacks.savingConfiguration") : t("fallbacks.saveAllConfigurations")}
            </Button>
          </div>
        )}
      </AddFallbacksModal>
