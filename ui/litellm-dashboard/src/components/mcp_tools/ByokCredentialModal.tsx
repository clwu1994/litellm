...
const byokSaveErrorMessage = (e: unknown, t: TFunction<"mcpServers">): string => {
  if (e instanceof ApiError) {
    const detail = (e.body as { detail?: { error?: string } } | null)?.detail?.error;
    if (detail) return detail;
  }
  return e instanceof Error && e.message ? e.message : t("byok.failedToConnect");
};
...
export const ByokCredentialModal: React.FC<ByokCredentialModalProps> = ({ server, open, onClose, onSuccess }) => {
  ...
  const serverDisplayName = server.alias || server.server_name || t("byok.fallbackName");
  ...
  const handleAuthorize = async () => {
    if (!apiKey.trim()) {
      toast.error(t("byok.enterApiKey"));
      return;
    }
    setLoading(true);
    try {
      ...
      toast.success(t("byok.connected", { name: serverDisplayName }));
        ...
        <div className="relative p-2">
          ...
          <div className="flex items-center justify-between mb-6">
            {step === 2 ? (
              ...
              >
                <ArrowLeft className="size-3.5" /> {t("byok.back")}
              </button>
            ) : (
              <div />
            )}
          ...
          {step === 1 ? (
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-foreground mb-2">
                {t("byok.connectTitle", { name: serverDisplayName })}
              </h2>
              <p className="text-muted-foreground mb-6">{t("byok.needsAccess", { name: serverDisplayName })}</p>
              ...
              <div className="bg-muted rounded-xl p-4 text-left mb-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-foreground mb-1">{t("byok.howItWorks")}</p>
                    <p className="text-muted-foreground text-sm">{t("byok.bridgeBody", { name: serverDisplayName })}</p>
                  </div>
                </div>
              </div>
              ...
              >
                {t("byok.continueToAuth")} <ArrowRight className="size-4" />
              </button>
              <button
                onClick={handleClose}
            ...
            <div>
              ...
              <h2 className="text-2xl font-bold text-foreground mb-2">{t("byok.provideKey")}</h2>
              <p className="text-muted-foreground mb-6">{t("byok.enterKeyFor", { name: serverDisplayName })}</p>

              <div className="mb-4">
                <label htmlFor={apiKeyInputId} className="block text-sm font-semibold text-foreground mb-2">
                  {t("byok.apiKeyLabel", { name: serverDisplayName })}
                </label>
                <PasswordInput
                  id={apiKeyInputId}
                ...
                {server.byok_api_key_help_url && (
                  ...
                  >
                    {t("byok.whereToFind")} <Link2 className="size-3.5" />
                  </a>
                )}
              </div>
              ...
              <div className="bg-muted rounded-xl p-4 flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-foreground">{t("byok.saveForFuture")}</span>
                </div>
                <Switch checked={saveKey} onCheckedChange={setSaveKey} aria-label={t("byok.saveForFuture")} />
              </div>

              {/* Security note */}
              <div className="bg-info/10 rounded-xl p-4 flex items-start gap-3 mb-6">
                <Lock className="mt-0.5 size-4 shrink-0 text-info" />
                <p className="text-sm text-info">{t("byok.securityNote")}</p>
              </div>
              ...
              >
                <Lock className="size-4" /> {t("byok.connectAuthorize")}
              </button>
            </div>
          )}
        </div>
