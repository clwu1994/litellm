      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="flex w-full items-center gap-3 px-4 py-3 text-left">
          ...
          <div className="flex items-center justify-between w-full">
            <h3 className="text-lg font-medium text-foreground">{t("cost.title")}</h3>
            <div className="flex items-center space-x-2 mr-4">
              <span className="text-sm text-muted-foreground">{t("cost.total")}</span>
              <span className="text-sm font-semibold text-foreground">
                {formatCost(totalSpend)}
                {isCached && t("cost.cached")}
              </span>
            </div>
          </div>
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div className="p-6 space-y-4">
            ...
            <div className="space-y-2 max-w-2xl">
                  ...
                  return (
                    <>
                      <div className="flex text-sm">
                        <span className="text-muted-foreground font-medium w-1/3">{t("cost.inputCost")}</span>
                        <span className="text-foreground">
                          {formatCost(rawCost)}
                          {rawInputTokens !== undefined && rawInputTokens !== null && (
                            <span className="text-muted-foreground font-normal ml-1">
                              {t("cost.tokens", { value: rawInputTokens.toLocaleString() })}
                            </span>
                          )}
                        </span>
                      </div>
                      {(costBreakdown?.cache_read_cost ?? 0) > 0 && (
                        <div className="flex text-sm">
                          <span className="text-muted-foreground font-medium w-1/3">
                            {t("cost.promptCacheReadCost")}
                          </span>
                          <span className="text-foreground">
                            {formatCost(isCached ? 0 : costBreakdown?.cache_read_cost)}
                            {(cacheReadTokens ?? 0) > 0 && (
                              <span className="text-muted-foreground font-normal ml-1">
                                {t("cost.tokens", { value: (cacheReadTokens ?? 0).toLocaleString() })}
                              </span>
                            )}
                          </span>
                        </div>
                      ...
                      {(costBreakdown?.cache_creation_cost ?? 0) > 0 && (
                        <div className="flex text-sm">
                          <span className="text-muted-foreground font-medium w-1/3">
                            {t("cost.promptCacheWriteCost")}
                          </span>
                          <span className="text-foreground">
                            {formatCost(isCached ? 0 : costBreakdown?.cache_creation_cost)}
                            {(cacheCreationTokens ?? 0) > 0 && (
                              <span className="text-muted-foreground font-normal ml-1">
                                {t("cost.tokens", { value: (cacheCreationTokens ?? 0).toLocaleString() })}
                              </span>
                            )}
                          </span>
                        </div>
                  ...
                  <div className="flex text-sm">
                    ...
                    <span className="text-foreground">
                      ...
                      {promptTokens !== undefined && (
                        <span className="text-muted-foreground font-normal ml-1">
                          {t("cost.promptTokens", { value: promptTokens.toLocaleString() })}
                        </span>
                      )}
                    </span>
                  </div>
              ...
              <div className="flex text-sm">
                <span className="text-muted-foreground font-medium w-1/3">{t("cost.outputCost")}</span>
                <span className="text-foreground">
                  {formatCost(outputCost)}
                  {completionTokens !== undefined && (
                    <span className="text-muted-foreground font-normal ml-1">
                      {t("cost.completionTokens", { value: completionTokens.toLocaleString() })}
                    </span>
                  )}
                </span>
              </div>
              {costBreakdown?.tool_usage_cost !== undefined && costBreakdown.tool_usage_cost > 0 && (
                <div className="flex text-sm">
                  <span className="text-muted-foreground font-medium w-1/3">{t("cost.toolUsageCost")}</span>
            ...
            {!isCached && (
              <div className="pt-2 border-t border-border max-w-2xl">
                <div className="flex text-sm font-semibold">
                  <span className="text-foreground w-1/3">{t("cost.originalLlmCost")}</span>
                ...
                {hasDiscount && (
                  <div className="space-y-2">
                    {costBreakdown.discount_percent !== undefined && costBreakdown.discount_percent !== 0 && (
                      <div className="flex text-sm text-muted-foreground">
                        <span className="font-medium w-1/3">
                          {t("cost.discount", { percent: formatPercent(costBreakdown.discount_percent) })}
                        </span>
                    ...
                    {costBreakdown.discount_amount !== undefined && costBreakdown.discount_percent === undefined && (
                      <div className="flex text-sm text-muted-foreground">
                        <span className="font-medium w-1/3">{t("cost.discountAmount")}</span>
                        <span className="text-foreground">-{formatCost(costBreakdown.discount_amount)}</span>
                      </div>
                    )}
                  </div>
                ...
                {hasMargin && (
                  <div className="space-y-2">
                    {costBreakdown.margin_percent !== undefined && costBreakdown.margin_percent !== 0 && (
                      <div className="flex text-sm text-muted-foreground">
                        <span className="font-medium w-1/3">
                          {t("cost.margin", { percent: formatPercent(costBreakdown.margin_percent) })}
                        </span>
                        <span className="text-foreground">
                          +
                          {formatCost(
                    ...
                    {costBreakdown.margin_fixed_amount !== undefined && costBreakdown.margin_fixed_amount !== 0 && (
                      <div className="flex text-sm text-muted-foreground">
                        <span className="font-medium w-1/3">{t("cost.marginAmount")}</span>
            ...
            <div className="mt-4 pt-4 border-t border-border max-w-2xl">
              <div className="flex items-center">
                <span className="font-bold text-sm text-foreground w-1/3">{t("cost.finalCalculatedCost")}</span>
