    ...
    <div className="bg-card rounded-lg shadow-sm w-full max-w-full overflow-hidden mb-6">
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="flex w-full items-center gap-3 px-4 py-3 text-left">
          ...
          <h3 className="text-lg font-medium text-foreground">{t("vectorStore.title")}</h3>
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div className="p-4">
            {data.map((request, index) => (
              <div key={index} className="mb-6 last:mb-0">
                <div className="bg-card rounded-lg border p-4 mb-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex">
                        <span className="font-medium w-1/3">{t("vectorStore.vectorStoreId")}</span>
                ...
                <h4 className="font-medium mb-2">{t("vectorStore.searchResults")}</h4>
                <div className="space-y-2">
                        ...
                        >
                          ...
                          <div className="flex items-center">
                            <span className="font-medium mr-2">
                              {t("vectorStore.result", { index: resultIndex + 1 })}
                            </span>
                            <span className="text-muted-foreground text-sm">
                              {t("vectorStore.scoreLabel")} <span className="font-mono">{result.score.toFixed(4)}</span>
                            </span>
                          </div>
                        </div>
