    ...
    <div className="border border-border rounded-lg bg-card">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {!loading && !error && data && (
            ...
            >
              {data.compliant ? t("guardrail.compliance.compliant") : t("guardrail.compliance.nonCompliant")}
            </span>
          )}
          {error && (
            <span className="px-2 py-0.5 rounded-sm text-[11px] font-medium bg-muted text-muted-foreground border border-border">
              {t("guardrail.compliance.unavailable")}
            </span>
          )}
          <svg
            width="20"
      ...
      {expanded && (
        <div className="border-t border-border px-4 py-3">
          {loading && <p className="text-sm text-muted-foreground">{t("guardrail.compliance.checking")}</p>}
...
const CompliancePanel: React.FC<CompliancePanelProps> = ({ accessToken, logEntry }) => {
  ...
  useEffect(() => {
    ...
    checkEuAiActCompliance(accessToken, payload)
      .then(setEuAiActData)
      .catch((err) => setEuAiActError(err.message || t("guardrail.compliance.euAiActError")))
    ...
    checkGdprCompliance(accessToken, payload)
      .then(setGdprData)
      .catch((err) => setGdprError(err.message || t("guardrail.compliance.gdprError")))
  ...
  return (
    <div>
      <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-4">
        {t("guardrail.compliance.title")}
      </h4>
      <div className="space-y-3">
        <ComplianceCard
          title={t("guardrail.compliance.euAiAct")}
          data={euAiActData}
          loading={euAiActLoading}
          error={euAiActError}
        />
        <ComplianceCard
          title={t("guardrail.compliance.gdpr")}
          data={gdprData}
          loading={gdprLoading}
          error={gdprError}
        />
