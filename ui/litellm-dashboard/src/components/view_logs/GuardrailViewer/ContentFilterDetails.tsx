...
const ContentFilterDetails: React.FC<ContentFilterDetailsProps> = ({ response }) => {
      ...
      return (
        <div className="bg-card rounded-lg border border-destructive/20 p-4">
          <div className="text-destructive">
            <h5 className="font-medium mb-2">{t("guardrail.contentFilter.error")}</h5>
  ...
  if (detections.length === 0) {
    return (
      <div className="bg-card rounded-lg border border-border p-4">
        <div className="text-muted-foreground text-sm">{t("guardrail.contentFilter.noDetections")}</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-card rounded-lg border border-border p-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <KV label={t("guardrail.contentFilter.totalDetections")}>
              <span className="font-semibold">{totalDetections}</span>
            </KV>
            <KV label={t("guardrail.contentFilter.actions")}>
          ...
          <div className="space-y-2">
            <KV label={t("guardrail.contentFilter.byType")}>
              <div className="flex flex-wrap gap-2">
                {patterns.length > 0 &&
                  chip(t("guardrail.contentFilter.patternsCount", { value: patterns.length }), "slate")}
                {blockedWords.length > 0 &&
                  chip(t("guardrail.contentFilter.keywordsCount", { value: blockedWords.length }), "slate")}
                {categoryKeywords.length > 0 &&
                  chip(t("guardrail.contentFilter.categoriesCount", { value: categoryKeywords.length }), "slate")}
              </div>
            </KV>
          </div>
        </div>
      ...
      {/* Patterns Section */}
      {patterns.length > 0 && (
        <Section title={t("guardrail.contentFilter.patternsMatched")} count={patterns.length} defaultOpen={true}>
          <div className="space-y-2">
            {patterns.map((detection, idx) => (
              <div key={idx} className="p-3 bg-muted rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("guardrail.contentFilter.pattern")}>
                      {detection.pattern_name || t("guardrail.contentFilter.unknown")}
                    </KV>
                  </div>
                  <div className="space-y-1">
                    <KV label={t("guardrail.contentFilter.action")}>
      ...
      {blockedWords.length > 0 && (
        <Section title={t("guardrail.contentFilter.blockedWords")} count={blockedWords.length} defaultOpen={true}>
          <div className="space-y-2">
            {blockedWords.map((detection, idx) => (
              <div key={idx} className="p-3 bg-muted rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("guardrail.contentFilter.keyword")} mono>
                      {detection.keyword || t("guardrail.contentFilter.unknown")}
                    </KV>
                    {detection.description && (
                      <KV label={t("guardrail.contentFilter.description")}>{detection.description}</KV>
                    )}
                  </div>
                  <div className="space-y-1">
                    <KV label={t("guardrail.contentFilter.action")}>
      ...
      {categoryKeywords.length > 0 && (
        <Section
          title={t("guardrail.contentFilter.categoryKeywords")}
          ...
          <div className="space-y-2">
            {categoryKeywords.map((detection, idx) => (
              <div key={idx} className="p-3 bg-muted rounded-md">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <KV label={t("guardrail.contentFilter.category")}>
                      {detection.category || t("guardrail.contentFilter.unknown")}
                    </KV>
                    <KV label={t("guardrail.contentFilter.keyword")} mono>
                      {detection.keyword || t("guardrail.contentFilter.unknown")}
                    </KV>
                    {detection.severity && (
                      <KV label={t("guardrail.contentFilter.severity")}>
                        {chip(
                          detection.severity,
      ...
      <Section title={t("guardrail.contentFilter.raw")} defaultOpen={false}>
