...
export function AgentPermissions({
  ...
  const mergedItems = [
    ...agents.map((agent) => ({
      type: "agent",
      value: agent,
      tooltip: t("permissions.fullId", { id: agent }),
    })),
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-foreground">{t("permissions.agents")}</p>
      ...
      {totalCount > 0 ? (
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted border border-border">
          ...
          <p className="text-muted-foreground text-sm">{t("permissions.noAgents")}</p>
        </div>
      )}
    </div>
  );
