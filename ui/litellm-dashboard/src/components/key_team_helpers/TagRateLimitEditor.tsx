    ...
    <div>
      {value.map((row, idx) => (
        <div key={row.id} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 12 }}>
          ...
          />
          <Input
            aria-label={t("budgets.rpmLimitLabel")}
            type="number"
            min={0}
            value={row.rpm_limit ?? ""}
            onChange={(e) => updateRow(idx, "rpm_limit", e.target.value === "" ? null : Number(e.target.value))}
            placeholder={t("budgets.rpmPlaceholder")}
            style={{ width: 120 }}
          />
          <Button
            variant="destructive"
            size="sm"
            aria-label={t("budgets.removeTagLimit")}
