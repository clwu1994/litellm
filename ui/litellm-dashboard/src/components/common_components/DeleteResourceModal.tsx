    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && !confirmLoading && onCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          ...
          {requiredConfirmation && (
            <div className="mb-6 mt-4 pt-4 border-t border-border">
              <p className="block text-base font-medium text-foreground mb-2">
                <Trans
                  ns="common"
                  i18nKey="deleteModal.confirmPrompt"
                  values={{ confirmation: requiredConfirmation }}
                  components={{ confirm: <span className="font-semibold text-destructive" /> }}
                />
              </p>
        ...
        <DialogFooter>
          ...
          >
            {confirmLoading ? t("deleteModal.deleting") : t("deleteModal.delete")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
