  ...
  if (variant === "button") {
    return (
      <Button onClick={startOAuthFlow} disabled={loading} className="font-semibold h-[38px] min-w-[110px]">
        ...
        {loading ? t("integrations.connecting") : t("common.connect")}
      </Button>
    );
  }
...
const MCPAppsPanel: React.FC<Props> = ({ accessToken, selectedServers, onChange, connectMode }) => {
  ...
  const connectUnavailabilityLabel = useCallback(
    (s: MCPServer): string | null => {
      ...
      if (isUnsupportedOnGatewayConnect(s.auth_type)) return t("integrations.notSupported");
      return null;
    },
    [connectMode, t],
  );
  ...
  const handleToggle = async (server: MCPServer, checked: boolean) => {
    ...
    try {
      ...
      if (result?.error) {
        toast.warning(t("integrations.toastToolsFailed", { server: serverName }));
        return;
      }
      if (connectableNow(server.server_id) === undefined) return;
  ...
  const emptyStateText = () => {
    if (servers.length === 0) {
      return connectMode ? t("integrations.emptyConnectMode") : t("integrations.emptyNoServers");
    }
    return activeTab === "connected" ? t("integrations.emptyNoConnected") : t("integrations.emptyNoMatch");
  };
  ...
  if (detailServer) {
    ...
    const renderDetailAction = () => {
      ...
      if (getMcpOAuthMode(detailServer) === "m2m") {
        return <span className="text-[13px] text-muted-foreground">{t("integrations.authorized")}</span>;
      }
      ...
      <div className="w-full">
        ...
        <div className="flex items-start gap-5 mb-7">
          ...
          <div className="flex-1">
            ...
            <p className="m-0 text-sm text-muted-foreground">
              {detailServer.description ?? t("integrations.mcpServerFallback")}
            </p>
          </div>
          {renderDetailAction()}
        </div>
        ...
        <div className="border rounded-lg overflow-hidden mb-7">
          {[
            ...
            [t("common.status"), isConnected ? t("common.connected") : t("common.notConnected")],
          ]
            .filter(([, v]) => v)
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
        <div>
          ...
          {connectMode ? (
            <p className="m-0 text-[13px] text-muted-foreground">{t("integrations.subtitleConnect")}</p>
          ) : (
            <div className="flex items-center gap-3">
              <p className="m-0 text-[13px] text-muted-foreground">{t("integrations.subtitleBrowse")}</p>
              {loadingCounts ? (
                ...
                <span className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  {totalTools === 1
                    ? t("integrations.toolCountOne", { total: totalTools })
                    : t("integrations.toolCountOther", { total: totalTools })}
                </span>
              ) : null}
            </div>
          )}
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabKey)} className="mb-4">
        <TabsList variant="line" className="border-b rounded-none w-full justify-start h-auto p-0">
          ...
          <TabsTrigger value="connected" className="rounded-none px-4 py-2 text-[13px]">
            {connectedCount > 0
              ? t("integrations.tabConnectedWithCount", { total: connectedCount })
              : t("common.connected")}
          </TabsTrigger>
        </TabsList>
      </Tabs>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1.5">
                    <span className="truncate">{server.description ?? t("integrations.mcpServerFallback")}</span>
                    {count !== undefined ? (
                      count > 0 ? (
