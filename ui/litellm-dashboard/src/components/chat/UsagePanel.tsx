...
const UsagePanel: React.FC<Props> = ({ accessToken, userId }) => {
  ...
  const statCards: Array<{ id: string; label: string; value: string; sub?: string; subVariant?: "error" }> = meta
    ? [
        { id: "totalSpend", label: t("usage.totalSpend"), value: `$${meta.total_spend.toFixed(2)}` },
        { id: "apiRequests", label: t("usage.apiRequests"), value: formatNumber(meta.total_api_requests) },
        {
          id: "tokensUsed",
          label: t("usage.tokensUsed"),
          value: formatNumber(meta.total_tokens),
          sub: t("usage.tokensBreakdown", {
