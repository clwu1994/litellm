...
const EndpointUsageTable: React.FC<EndpointUsageTableProps> = ({ endpointData }) => {
  ...
  const columns: ColumnDef<EndpointRow>[] = [
    {
      header: t("endpoint.table.endpoint"),
      accessorKey: "endpoint",
      cell: ({ row }) => <span className="font-medium">{row.original.endpoint}</span>,
    },
    {
      header: t("endpoint.table.successfulFailed"),
      id: "requests",
      cell: ({ row }) => {
          ...
          <div className="flex items-center space-x-3">
            <div className="flex-1 relative">
              <Meter
                ...
                aria-label={t("endpoint.table.successfulRequestsAria")}
              >
    ...
    {
      header: t("endpoint.table.totalTokens"),
      accessorKey: "total_tokens",
      meta: { numeric: true },
      cell: ({ row }) => row.original.total_tokens.toLocaleString(),
    },
    {
      header: t("endpoint.table.spend"),
      accessorKey: "spend",
      meta: { numeric: true },
  ...
  return (
    <DataTable
      ...
      noDataMessage={t("empty.noEndpointUsageData")}
      size="compact"
    />
  );
};
