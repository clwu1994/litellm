  ...
  return (
    <div className="min-w-40 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl">
      <p className="mb-1.5 font-medium text-foreground">
        {t("errorDrilldown.tooltip", {
          code: String(label),
          total: datum[FAILED_REQUESTS_SERIES].toLocaleString(),
        })}
      </p>
  ...
  return (
    <Card className="mt-4">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle className="text-base font-semibold">{t("errorDrilldown.title", { callType })}</CardTitle>
        <Button variant="outline" size="icon-sm" onClick={onClose} aria-label={t("errorDrilldown.close")}>
          <X />
        </Button>
      </CardHeader>
      <CardContent>
        <p className="text-sm text-muted-foreground">{t("errorDrilldown.hint")}</p>
        <BarChart
