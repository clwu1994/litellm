...
const UsagePage: React.FC<UsagePageProps> = ({ accessToken, token, userRole, userID, keys, premiumUser }) => {
  const { t } = useTranslation("oldUsage");
  ...
  const tagOptions: TagOption[] = [
    { value: ALL_TAGS, label: t("tagSpend.allTags"), disabled: false },
    ...allTagNames
      .filter((tag) => tag !== ALL_TAGS)
      .map((tag) => ({
        value: tag,
        label: premiumUser ? tag : t("tagSpend.enterpriseOnly", { tag }),
  ...
  if (!canViewGlobalSpend) {
    return (
      <div className="w-full p-8">
        <Card>
          <CardHeader>
            <CardTitle>{t("adminOnly.title")}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">{t("adminOnly.body")}</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  if (proxySettings?.DISABLE_EXPENSIVE_DB_QUERIES) {
    return (
      <div className="w-full p-8">
        <Card>
          <CardHeader>
            <CardTitle>{t("dbLimit.title")}</CardTitle>
          </CardHeader>
          <CardContent className="flex flex-col items-start gap-4">
            <p className="text-sm text-muted-foreground">
              {t("dbLimit.rows", { rows: proxySettings.NUM_SPEND_LOGS_ROWS })}
              <br></br>
              {t("dbLimit.guide")}
            </p>
            <Button
    ...
    <div className="w-full p-8">
      <Tabs defaultValue="all-up">
        <TabsList variant="line" className="mt-2">
          <TabsTrigger value="all-up">{t("tabs.allUp")}</TabsTrigger>

          {isAdminOrAdminViewer(userRole) && (
            <>
              <TabsTrigger value="team-based-usage">{t("tabs.teamBased")}</TabsTrigger>
              <TabsTrigger value="customer-usage">{t("tabs.customer")}</TabsTrigger>
              <TabsTrigger value="tag-based-usage">{t("tabs.tagBased")}</TabsTrigger>
            </>
          )}
        </TabsList>
        ...
        <TabsContent value="all-up" keepMounted>
          <Tabs defaultValue="cost">
            <TabsList className="mt-1">
              <TabsTrigger value="cost">{t("costTab.cost")}</TabsTrigger>
              <TabsTrigger value="activity">{t("costTab.activity")}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="cost" keepMounted>
              <div className="grid h-screen w-full grid-cols-2 gap-2">
                ...
                <div className="col-span-2">
                  <Card>
                    <CardHeader>
                      <CardTitle>{t("cost.monthlySpend")}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <BarChart
                        data={keySpendData}
                ...
                <div className="col-span-1">
                  <Card className="h-full">
                    <CardHeader>
                      <CardTitle>{t("cost.topVirtualKeys")}</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <div className="col-span-1">
                  <Card className="h-full">
                    <CardHeader>
                      <CardTitle>{t("cost.topModels")}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <BarChart
                        className="mt-4 h-40"
                ...
                <div className="col-span-2">
                  <Card className="mb-2">
                    <CardHeader>
                      <CardTitle>{t("cost.spendByProvider")}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-2">
                        ...
                        <div className="col-span-1">
                          <Table>
                            <TableHeader>
                              <TableRow>
                                <TableHead>{t("cost.providerColumn")}</TableHead>
                                <TableHead>{t("cost.spendColumn")}</TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                    ...
                    <div className="grid grid-cols-2">
                      <div>
                        <p className="text-[15px] font-normal text-muted-foreground">
                          {t("activity.apiRequests", { value: valueFormatterNumbers(globalActivity.sum_api_requests) })}
                        </p>
                        <AreaChart
                          className="h-40"
                      ...
                      <div>
                        <p className="text-[15px] font-normal text-muted-foreground">
                          {t("activity.tokens", { value: valueFormatterNumbers(globalActivity.sum_total_tokens) })}
                        </p>
                        <BarChart
                          className="h-40"
                          data={globalActivity.daily_data}
                  ...
                  <Card key={index}>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2">
                        <div>
                          <p className="text-[15px] font-normal text-muted-foreground">
                            {t("activity.apiRequests", {
                              value: valueFormatterNumbers(globalActivity.sum_api_requests),
                            })}
                          </p>
                          <AreaChart
                            className="h-40"
        ...
        <TabsContent value="team-based-usage" keepMounted>
          <div className="grid h-[75vh] w-full grid-cols-2 gap-2">
            <div className="col-span-2">
              <Card className="mb-2">
                <CardHeader>
                  <CardTitle>{t("teamSpend.totalPerTeam")}</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>{t("teamSpend.dailyPerTeam")}</CardTitle>
                </CardHeader>
                <CardContent>
                  <BarChart
                    className="h-72"
        ...
        <TabsContent value="customer-usage" keepMounted>
          ...
          <div className="grid grid-cols-2">
            ...
            <div>
              <p className="text-sm text-muted-foreground">{t("customer.selectKey")}</p>
              <Select
              ...
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={t("customer.allKeys")}>
                    {(token: string | null) =>
                      selectableKeys.find((key) => key.token === token)?.alias ?? t("customer.allKeys")
                    }
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={null}>{t("customer.allKeys")}</SelectItem>
            ...
            <CardContent>
              <div className="max-h-[70vh] min-h-[500px] overflow-y-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{t("customer.customerColumn")}</TableHead>
                      <TableHead>{t("customer.spendColumn")}</TableHead>
                      <TableHead>{t("customer.totalEventsColumn")}</TableHead>
                    </TableRow>
                  </TableHeader>
        ...
        <TabsContent value="tag-based-usage" keepMounted>
            ...
            <div>
              ...
              >
                <ComboboxChips render={<div ref={anchor} />}>
                  ...
                  <ComboboxChipsInput placeholder={t("tagSpend.selectTags")} />
                </ComboboxChips>
                <ComboboxContent anchor={anchor}>
                  <ComboboxEmpty>{t("tagSpend.noTags")}</ComboboxEmpty>
                  <ComboboxList>
                    {(option: TagOption) => (
          ...
          <div className="mb-4 grid h-[75vh] w-full grid-cols-2 gap-2">
            <div className="col-span-2">
              <Card>
                <CardHeader>
                  <CardTitle>{t("tagSpend.title")}</CardTitle>
                </CardHeader>
                <CardContent className="flex flex-col gap-2">
                  <p className="text-sm text-muted-foreground">
                    {t("tagSpend.getStarted")}{" "}
                    <a
                      className="text-primary"
