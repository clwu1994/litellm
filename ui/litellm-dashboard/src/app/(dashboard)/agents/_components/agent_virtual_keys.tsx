...
const AgentVirtualKeys: React.FC<AgentVirtualKeysProps> = ({ keys, isLoading, onKeyClick }) => {
  ...
  return (
    <div className="mt-6">
      <h4 className="text-base font-semibold text-foreground">{t("keys.title")}</h4>
      {isLoading ? (
        <p className="mt-2 text-sm text-muted-foreground">{t("keys.loading")}</p>
      ) : keys.length === 0 ? (
        <p className="mt-2 text-sm text-muted-foreground">{t("keys.empty")}</p>
      ) : (
        <div className="mt-3 flex flex-col gap-2">
          {keys.map((key) => (
            <div key={key.token} className="flex items-center gap-3 rounded-sm border border-border px-3 py-2">
              <KeyRound className="size-4 text-muted-foreground" />
              <span className="text-sm font-medium text-foreground">{key.key_alias || t("keys.unnamed")}</span>
