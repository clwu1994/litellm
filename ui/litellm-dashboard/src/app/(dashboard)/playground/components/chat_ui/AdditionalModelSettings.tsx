      ...
      {onStreamingChange && (
        <div className="flex items-center gap-2">
          ...
          <label htmlFor={streamingId} className="cursor-pointer text-sm font-medium">
            {t("settings.streamResponses")}
          </label>
          <Tooltip>
            <TooltipTrigger aria-label={t("settings.streamResponsesHelpAria")}>
              <Info className="size-3 shrink-0 cursor-pointer text-muted-foreground hover:text-foreground" />
            </TooltipTrigger>
            <TooltipContent className="max-w-xs">{t("settings.streamResponsesTooltip")}</TooltipContent>
          </Tooltip>
        </div>
      )}
      ...
      {onMockTestFallbacksChange && (
        <div className="flex items-center gap-2">
          <Checkbox
            ...
            aria-label={t("settings.simulateFallbacks")}
          />
          <label htmlFor={fallbacksId} className="cursor-pointer text-sm font-medium">
            {t("settings.simulateFallbacks")}
          </label>
          <Popover>
            <PopoverTrigger aria-label={t("settings.simulateFallbacksHelpAria")}>
              <Info className="size-3 shrink-0 cursor-pointer text-muted-foreground hover:text-foreground" />
            </PopoverTrigger>
            <PopoverContent side="right" className="max-w-[340px] gap-2 p-3 text-sm">
              <p>{t("settings.simulateFallbacksBody")}</p>
              <p>
                <Trans
                  ns="playground"
                  i18nKey="settings.simulateFallbacksNote"
                  components={{
                    docs: (
                      <a
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              <div className="flex items-center gap-1">
                ...
                <Tooltip>
                  <TooltipTrigger aria-label={t("settings.temperatureHelpAria")}>
                    <Info className={cn("size-3 cursor-help", disabledTextColor)} />
                  </TooltipTrigger>
                  <TooltipContent className="max-w-xs">{t("settings.temperatureTooltip")}</TooltipContent>
                </Tooltip>
              </div>
              <Input
                ...
                type="text"
                inputMode="decimal"
                aria-label={t("settings.temperatureValue")}
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              <div className="flex items-center gap-1">
                ...
                <Tooltip>
                  <TooltipTrigger aria-label={t("settings.maxTokensHelpAria")}>
                    <Info className={cn("size-3 cursor-help", disabledTextColor)} />
                  </TooltipTrigger>
                  <TooltipContent className="max-w-xs">{t("settings.maxTokensTooltip")}</TooltipContent>
                </Tooltip>
              </div>
              <Input
                ...
                type="text"
                inputMode="numeric"
                aria-label={t("settings.maxTokensValue")}
                value={maxTokensText}
                disabled={!useAdvancedParams}
                className="h-8 w-24"
