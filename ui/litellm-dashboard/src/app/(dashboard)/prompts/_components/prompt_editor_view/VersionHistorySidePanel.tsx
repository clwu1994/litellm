  ...
  return (
    ...
    >
      <Button type="button" variant="ghost" size="icon-sm" className="absolute top-4 right-4" onClick={onClose}>
        <XIcon />
        <span className="sr-only">{t("versions.close")}</span>
      </Button>
      ...
      <div className="overflow-y-auto px-4 pb-4">
        {loading ? (
          <div className="space-y-3" role="status" aria-label={t("versions.loadingAria")}>
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {index === 0 && <Badge>{t("versions.latest")}</Badge>}
                    </div>
                    {isSelected && <Badge variant="secondary">{t("versions.active")}</Badge>}
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {item.prompt_info?.prompt_type === "db"
                        ? t("versions.savedToDatabase")
                        : t("versions.configPrompt")}
                    </span>
                  </div>
                </button>
              );
