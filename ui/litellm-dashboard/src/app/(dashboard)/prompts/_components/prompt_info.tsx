...
const PromptInfoView: React.FC<PromptInfoProps> = ({
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t("info.toast.deleted", { promptId: basePromptId }));
      onDelete?.();
      onClose();
    } catch (error) {
      console.error("Error deleting prompt:", error);
      toast.fromError(t("info.toast.deleteFailed"));
    } finally {
      setIsDeleting(false);
  ...
  const handleVersionClick = async (version: PromptSpec) => {
    ...
    try {
      ...
      toast.fromError(t("info.toast.versionLoadFailed", { version: versionNum }));
    }
  };
    ...
    <div className="p-4">
      ...
      {isViewingOldVersion && (
        <div className="mb-4 p-3 bg-warning/10 border border-warning/20 rounded-lg flex items-center justify-between">
          <p className="text-sm text-warning">
            {t("info.oldVersionBanner", { version: selectedVersion, latestVersion })}
          </p>
          <Button
            variant="ghost"
            size="sm"
        ...
        <div>
          ...
          <TabsContent value="overview" keepMounted>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Card className="block p-6">
                <p>{t("info.fields.createdAt")}</p>
                <div className="mt-2">
                  <h3 className="text-sm font-medium">{formatDate(promptData.created_at)}</h3>
                  <p className="text-xs">{t("info.fields.updated", { date: formatDate(promptData.updated_at) })}</p>
                </div>
              </Card>
            </div>
            ...
            <Card className="block mt-6 p-6">
              <h3 className="text-lg font-medium mb-3">
                {t("info.versionHistory.title", { environment: translateEnvironment(t, selectedEnv) })}
              </h3>
              {loadingVersions ? (
                <p>{t("info.versionHistory.loading")}</p>
              ) : versionHistory.length > 0 ? (
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{t("info.versionHistory.columns.version")}</TableHead>
                      <TableHead>{t("info.versionHistory.columns.createdBy")}</TableHead>
                      <TableHead>{t("info.versionHistory.columns.date")}</TableHead>
                      <TableHead>{t("info.versionHistory.columns.actions")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                ...
                <p className="text-muted-foreground">
                  {t("info.versionHistory.empty", { environment: translateEnvironment(t, selectedEnv) })}
                </p>
              )}
            </Card>
          </TabsContent>
            ...
            <TabsContent value="prompt-template" keepMounted>
              <Card className="block p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium">{t("info.template.title")}</h3>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  >
                    ...
                    {copiedStates["prompt-content"] ? t("info.template.copied") : t("info.template.copyContent")}
                  </Button>
                </div>

                <div className="space-y-4">
                  <div>
                    <p className="font-medium">{t("info.template.templateId")}</p>
                    <div className="font-mono text-sm bg-muted p-2 rounded-sm">{promptTemplate.litellm_prompt_id}</div>
                  </div>

                  <div>
                    <p className="font-medium">{t("info.template.content")}</p>
                  ...
                  {promptTemplate.metadata && Object.keys(promptTemplate.metadata).length > 0 && (
                    <div>
                      <p className="font-medium">{t("info.template.metadata")}</p>
          ...
          <TabsContent value="raw-json" keepMounted>
            <Card className="block p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">{t("info.rawJson.title")}</h3>
                <Button
                  variant="ghost"
                  size="sm"
                ...
                >
                  ...
                  {copiedStates["raw-json"] ? t("info.rawJson.copied") : t("info.rawJson.copyJson")}
                </Button>
              </div>
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={(open) => !open && handleDeleteCancel()}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("info.deleteModal.title")}</DialogTitle>
          </DialogHeader>
          <p>
            <Trans
              ns="prompts"
              i18nKey="info.deleteModal.message"
              values={{ promptId: basePromptId }}
              components={{ strong: <strong /> }}
            />
          </p>
          <p>{t("info.deleteModal.warning")}</p>
          <DialogFooter>
            <Button variant="outline" onClick={handleDeleteCancel}>
              {t("info.deleteModal.cancel")}
            </Button>
            <Button onClick={handleDeleteConfirm} variant="destructive" disabled={isDeleting} aria-busy={isDeleting}>
              {t("info.deleteModal.delete")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
