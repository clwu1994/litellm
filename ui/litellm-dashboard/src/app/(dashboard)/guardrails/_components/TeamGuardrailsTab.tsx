...
const GUARDRAIL_MODES = [
  { value: "pre_call", labelKey: "teamTab.modes.preCall" },
  { value: "post_call", labelKey: "teamTab.modes.postCall" },
  { value: "during_call", labelKey: "teamTab.modes.duringCall" },
] as const satisfies ReadonlyArray<{ value: string; labelKey: ParseKeys<"guardrails"> }>;

const createSubmitGuardrailSchema = (t: TFunction<"guardrails">) =>
  z.object({
    team_id: z
      .string()
      .nullable()
      .pipe(z.string({ error: t("teamTab.validation.selectTeam") }).min(1, t("teamTab.validation.selectTeam"))),
    guardrail_name: z.string().min(1, t("teamTab.validation.nameRequired")),
    mode: z.string().min(1, "Select a mode"),
    api_base: z
      .string()
      .min(1, t("teamTab.validation.apiBaseRequired"))
      .refine(isValidUrl, t("teamTab.validation.validUrl")),
    extra_litellm_params: z.string().superRefine((value, ctx) => {
      if (!value) return;
      try {
        const parsed: unknown = JSON.parse(value);
        if (typeof parsed !== "object" || Array.isArray(parsed)) {
          ctx.addIssue({ code: "custom", message: t("teamTab.validation.jsonObject") });
        }
      } catch {
        ctx.addIssue({ code: "custom", message: t("teamTab.validation.invalidJson") });
      }
    }),
...
const STATUS_CONFIG: Record<
  GuardrailStatus,
  { labelKey: ParseKeys<"guardrails">; bg: string; text: string; dot: string }
> = {
  active: {
    labelKey: "teamTab.status.active",
    bg: "bg-success/10",
    text: "text-success",
    dot: "bg-success",
  },
  pending: {
    labelKey: "teamTab.status.pending",
    bg: "bg-warning/10",
    text: "text-warning",
    dot: "bg-warning",
  },
  rejected: {
    labelKey: "teamTab.status.rejected",
    ...
    >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${teamColor}`}>
              {t("teamTab.card.team", { team: g.team })}
            </span>
            <span
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span>
              {t("teamTab.card.model")} <span className="font-medium text-foreground">{g.model}</span>
            </span>
            <span>
              {t("teamTab.card.submitted")} <span className="font-medium text-foreground">{g.submittedAt}</span>
            </span>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2 shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground whitespace-nowrap">{t("teamTab.card.forwardApiKey")}</span>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            >
              {isSelected ? t("teamTab.card.close") : t("teamTab.card.review")}
            </button>
      ...
      <div className="mt-3 pt-3 border-t border-border">
        ...
        {isHeadersExpanded && (
          <div className="mt-2">
            {g.customHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic">{t("teamTab.card.noStaticHeaders")}</p>
            ) : (
    ...
    <div className="w-96 shrink-0 bg-card overflow-auto">
      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {t("teamTab.detail.submittedBy", { user: g.submittedBy, date: g.submittedAt })}
            </p>
          </div>
          <button
            type="button"
            ...
            aria-label={t("teamTab.detail.closeAria")}
          >
        ...
        <div className="space-y-4">
          <ConfigRow label={t("teamTab.detail.endpoint")}>
          ...
          <ConfigRow label={t("teamTab.detail.method")}>
          ...
          <div className="border border-info/15 bg-info/10 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-info">{t("teamTab.detail.forwardApiKeyTitle")}</span>
              </div>
              <Toggle enabled={g.forwardKey} onToggle={onToggleForwardKey} disabled={!isAdmin} />
            </div>
            <p className="text-xs text-info leading-relaxed">
              <Trans
                ns="guardrails"
                i18nKey="teamTab.detail.forwardApiKeyBody"
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-xs font-semibold text-foreground">{t("teamTab.card.staticHeaders")}</span>
              {g.customHeaders.length > 0 && (
            ...
            <p className="text-xs text-muted-foreground mb-2">{t("teamTab.detail.staticHeadersHint")}</p>
            {g.customHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic mb-2">{t("teamTab.card.noStaticHeaders")}</p>
            ) : (
                  ...
                  >
                    ...
                    {isAdmin && (
                      <button
                        ...
                        aria-label={t("teamTab.detail.removeAria", { key: h.key })}
                      >
            ...
            {isAdmin && (
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
                <input
                  ...
                  placeholder={t("teamTab.detail.headerNamePlaceholder")}
                ...
                <input
                  ...
                  placeholder={t("teamTab.detail.valuePlaceholder")}
                  className="flex-1 min-w-0 text-xs font-mono border border-border rounded-sm px-2 py-1.5 text-foreground placeholder:text-muted-foreground focus:outline-hidden focus:ring-1 focus:ring-ring"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-xs font-semibold text-foreground">{t("teamTab.detail.forwardClientHeaders")}</span>
            ...
            <p className="text-xs text-muted-foreground mb-2">{t("teamTab.detail.forwardClientHeadersHint")}</p>
            {g.extraHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic mb-2">{t("teamTab.detail.noForwardClientHeaders")}</p>
            ) : (
              <ul className="list-none space-y-1 mb-2">
                  ...
                  >
                    ...
                    {isAdmin && (
                      <button
                        ...
                        className="text-muted-foreground hover:text-destructive shrink-0"
                        aria-label={t("teamTab.detail.removeAria", { key: name })}
                      >
                        <XIcon className="h-3.5 w-3.5" />
                      </button>
                    )}
          ...
          <div className="border border-border rounded-lg overflow-hidden">
            ...
            >
              <span>{t("teamTab.detail.equivalentConfig")}</span>
              {configExpanded ? (
          ...
          <div className="flex items-start gap-2 bg-muted border border-border rounded-lg p-3">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              <Trans
                ns="guardrails"
                i18nKey="teamTab.detail.docsBody"
                components={{
                  docs: (
                    <a
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-overlay">
      <div className="bg-card rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
        ...
        <h3 className="text-base font-semibold text-foreground mb-1">
          {isApprove ? t("teamTab.confirm.approveTitle") : t("teamTab.confirm.rejectTitle")}
        </h3>
        <p className="text-sm text-muted-foreground mb-5">
          {t("teamTab.confirm.sure", {
            action: isApprove ? t("teamTab.confirm.approveAction") : t("teamTab.confirm.rejectAction"),
          })}{" "}
          <span className="font-medium text-foreground">&quot;{guardrailName}&quot;</span>?{" "}
          {isApprove ? t("teamTab.confirm.approveDetail") : t("teamTab.confirm.rejectDetail")}
        </p>
        <div className="flex gap-3">
          ...
          >
            {isApprove ? t("teamTab.card.approve") : t("teamTab.card.reject")}
          </button>
        </div>
      </div>
    </div>
...
export function TeamGuardrailsTab({ accessToken }: TeamGuardrailsTabProps) {
  ...
  const modeItems = useMemo(() => GUARDRAIL_MODES.map(({ labelKey, value }) => ({ label: t(labelKey), value })), [t]);
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("teamTab.loadFailed"));
      setGuardrails([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmitGuardrail = submitForm.handleSubmit(async (values) => {
    ...
    try {
      ...
      toast.success(t("teamTab.toast.submitted"));
  ...
  async function toggleForwardKey(id: string) {
    ...
    try {
      ...
      toast.success(t(newValue ? "teamTab.toast.forwardKeyEnabled" : "teamTab.toast.forwardKeyDisabled"));
    } catch {
      toast.fromError(t("teamTab.toast.forwardKeyFailed"));
    }
  }
  ...
  async function updateCustomHeaders(id: string, customHeaders: { key: string; value: string }[]) {
    ...
    try {
      ...
      toast.success(t("teamTab.toast.staticHeadersUpdated"));
    } catch {
      toast.fromError(t("teamTab.toast.staticHeadersFailed"));
    }
  }
  ...
  async function updateExtraHeaders(id: string, extraHeaders: string[]) {
    ...
    try {
      ...
      toast.success(t("teamTab.toast.extraHeadersUpdated"));
    } catch {
      toast.fromError(t("teamTab.toast.extraHeadersFailed"));
    }
  }
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      toast.success(t("teamTab.toast.approved"));
    } catch {
      toast.fromError(t("teamTab.toast.approveFailed"));
    }
  }
  ...
  async function handleReject(id: string) {
    ...
    try {
      ...
      if (selectedId === id) setSelectedId(null);
      await fetchSubmissions();
      toast.success(t("teamTab.toast.rejected"));
    } catch {
      toast.fromError(t("teamTab.toast.rejectFailed"));
    }
  }
    ...
    <div className="flex h-full">
      <div className={`flex-1 min-w-0 p-6 overflow-auto ${selected ? "border-r border-border" : ""}`}>
        <div className="grid grid-cols-4 gap-4 mb-6">
          <StatCard label={t("teamTab.stats.totalSubmitted")} value={totalCount} color="text-foreground" />
          <StatCard label={t("teamTab.status.pending")} value={pendingCount} color="text-warning" />
          <StatCard label={t("teamTab.status.active")} value={activeCount} color="text-success" />
          <StatCard label={t("teamTab.status.rejected")} value={rejectedCount} color="text-destructive" />
        </div>
        <div className="flex items-center gap-3 mb-5">
          ...
          <select
            aria-label={t("teamTab.filterAria")}
            value={statusFilter}
          ...
          >
            <option value="all">{t("teamTab.filterAll")}</option>
            <option value="pending">{t("teamTab.status.pending")}</option>
            <option value="active">{t("teamTab.status.active")}</option>
            <option value="rejected">{t("teamTab.status.rejected")}</option>
          </select>
          <button
            type="button"
          ...
          >
            ...
            {t("teamTab.addGuardrail")}
          </button>
        </div>
        <div className="space-y-3">
          {isLoading && <div className="text-center py-12 text-muted-foreground text-sm">{t("teamTab.loading")}</div>}
          {error && <div className="text-center py-12 text-destructive text-sm">{error}</div>}
          {!isLoading && !error && filtered.length === 0 && (
            <div className="text-center py-12 text-muted-foreground text-sm">{t("teamTab.empty")}</div>
          )}
          {!isLoading &&
            !error &&
            filtered.map((g) => (
      ...
      >
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("teamTab.submit.title")}</DialogTitle>
          </DialogHeader>
          <div className="rounded-md bg-info/10 border border-info/20 px-4 py-3 text-sm text-info mb-4">
            {t("teamTab.submit.banner")}
          </div>
          <TooltipProvider>
            <form onSubmit={handleSubmitGuardrail}>
              <FieldGroup>
                <FormField control={submitForm.control} name="team_id" label={t("teamTab.submit.team")}>
                  {({ id, value, onChange }) => <TeamDropdown id={id} value={value} onChange={onChange} />}
                </FormField>
                <FormField control={submitForm.control} name="guardrail_name" label={t("teamTab.submit.guardrailName")}>
                  {({ ref, ...field }) => <Input {...field} ref={ref} placeholder="e.g. pii-detection" />}
                </FormField>
                <FormField control={submitForm.control} name="mode" label={t("teamTab.submit.mode")}>
                  {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
                    <Select items={modeItems} value={value} onValueChange={onChange}>
                      <SelectTrigger
                        id={id}
                ...
                <FormField control={submitForm.control} name="api_base" label={t("teamTab.submit.apiBaseUrl")}>
                  {({ ref, ...field }) => (
                    <Input
                      {...field}
                      ref={ref}
                ...
                <FormField
                  ...
                  label={labelWithHint(t("teamTab.submit.extraParams"), t("teamTab.submit.extraParamsHint"))}
                >
                  {({ ref, ...field }) => (
                    <Textarea
                      {...field}
                ...
                <FormField control={submitForm.control} name="guardrail_info" label={t("teamTab.submit.guardrailInfo")}>
                  {({ ref, ...field }) => (
                    <Textarea
                      {...field}
                      ref={ref}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmitGuardrail}>{t("teamTab.submit.submit")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
