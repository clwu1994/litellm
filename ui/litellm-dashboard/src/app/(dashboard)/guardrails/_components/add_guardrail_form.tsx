...
const MODE_DESCRIPTION_KEYS: Record<string, ParseKeys<"guardrails"> | undefined> = {
  pre_call: "form.modeDescriptions.preCall",
  during_call: "form.modeDescriptions.duringCall",
  post_call: "form.modeDescriptions.postCall",
  logging_only: "form.modeDescriptions.loggingOnly",
  pre_mcp_call: "form.modeDescriptions.preMcpCall",
  during_mcp_call: "form.modeDescriptions.duringMcpCall",
  post_mcp_call: "form.modeDescriptions.postMcpCall",
};

const modeDescription = (mode: string, t: TFunction<"guardrails">): string | undefined => {
  const key = MODE_DESCRIPTION_KEYS[mode];
...
const AddGuardrailForm: React.FC<AddGuardrailFormProps> = ({ visible, onClose, accessToken, onSuccess, preset }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        toast.fromError(t("form.toast.loadConfigFailed"));
      }
    };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!(await form.trigger())) {
        toast.fromError(t("form.validation.fixHighlighted"));
        return;
      }
      ...
      if (shouldRenderContentFilterConfigSettings(providerKey)) {
        ...
        if (!hasContentFilterSelections && !hasCompetitorIntent) {
          toast.fromError(t("form.validation.contentFilterRequired"));
          setLoading(false);
          return;
        }
      ...
      // Add config values to the guardrail_info if provided
        ...
        try {
          ...
          toast.fromError(t("form.validation.invalidJson"));
          setLoading(false);
          return;
        }
      }
      ...
      if (guardrailProvider === "llm_as_a_judge") {
        ...
        if (criteria.length === 0) {
          toast.fromError(t("form.validation.criteriaRequired"));
          setLoading(false);
          return;
        }
        const weightTotal = criteria.reduce((sum, c) => sum + (Number(c?.weight) || 0), 0);
        if (weightTotal !== 100) {
          toast.fromError(t("form.validation.criteriaWeightSum", { value: weightTotal }));
          setLoading(false);
          return;
        }
      ...
      if (guardrailProvider === "tool_permission") {
        if (toolPermissionConfig.rules.length === 0) {
          toast.fromError(t("form.validation.toolRuleRequired"));
          setLoading(false);
          return;
        }
      ...
      toast.fromError(
        t("form.toast.createFailed", { message: error instanceof Error ? error.message : String(error) }),
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <FieldGroup>
        <GuardrailField
          ...
          name="guardrail_name"
          label={t("form.guardrailName")}
          rules={requiredRule(t("form.validation.nameRequired"))}
        >
          {({ ref, value, ...field }) => (
            <Input {...field} ref={ref} value={asText(value)} placeholder={t("form.guardrailNamePlaceholder")} />
          )}
        </GuardrailField>
        ...
        <GuardrailField
          control={form.control}
          name="provider"
          label={t("form.guardrailProvider")}
          rules={requiredRule(t("form.validation.providerRequired"))}
        >
          {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
            ...
            >
              ...
              />
              <ComboboxContent>
                <ComboboxEmpty>{t("form.noMatchingProviders")}</ComboboxEmpty>
                <ComboboxList>
                  {(key: string) => (
        ...
        <GuardrailField
          control={form.control}
          name="mode"
          label={labelWithHint(t("form.mode"), t("form.modeHint"))}
          rules={requiredRule(t("form.validation.modeRequired"))}
        >
          {({ id, value, onChange }) => (
            <MultiSelect
              id={id}
              options={supportedModes.map((mode) => ({
                label: mode,
                value: mode,
                description: modeDescription(mode, t),
              }))}
        ...
        <GuardrailField
          control={form.control}
          name="default_on"
          label={labelWithHint(t("form.alwaysOn"), t("form.alwaysOnHint"))}
        >
          {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
            <Select
              items={alwaysOnItems}
        ...
        <GuardrailField
          ...
          label={labelWithHint(t("form.skipSystemMessageLabel"), t("form.skipSystemMessageHint"))}
        >
        ...
        <GuardrailField
          ...
          label={labelWithHint(t("form.skipToolMessageLabel"), t("form.skipToolMessageHint"))}
        >
          {(fieldControl) => <SkipMessageSelect control={fieldControl} />}
        </GuardrailField>
      ...
      <div className="space-y-6">
        <div>
          <p className="text-sm text-muted-foreground">
            <Trans ns="guardrails" i18nKey="form.endpoint.description" components={{ code: <code /> }} />
          </p>
        </div>

        <div>
          <label htmlFor="guardrail-call-type" className="mb-1 block text-sm font-medium text-foreground">
            {t("form.endpoint.callType")}
          </label>
          <Select
          ...
          >
            <SelectTrigger id="guardrail-call-type" className="w-65">
              <SelectValue placeholder={t("form.endpoint.callTypePlaceholder")} />
            </SelectTrigger>
            <SelectContent>
          ...
          <p className="mt-1 text-xs text-muted-foreground">{t("form.endpoint.moreCallTypes")}</p>
        </div>
        ...
        {selectedEndpointType === "realtime" && (
          <div className="overflow-hidden rounded-lg border border-border">
            ...
            >
              <span>{t("form.endpoint.realtimeSettings")}</span>
              <svg
            ...
            {endpointSettingsOpen && (
              <div className="space-y-5 border-t border-border px-4 py-4">
                <div>
                  ...
                  >
                    {t("form.endpoint.endSessionAfter")}
                  </label>
                  <p className="mb-2 text-xs text-muted-foreground">{t("form.endpoint.endSessionAfterHint")}</p>
                  <Input
                    id="guardrail-end-session-after"
                    type="number"
                    min={1}
                    placeholder={t("form.endpoint.endSessionPlaceholder")}
                      ...
                      <label key={opt} className="flex items-start gap-2 cursor-pointer">
                        ...
                        <div>
                          <span className="text-sm font-medium text-foreground">
                            {opt === "warn"
                              ? t("form.endpoint.onViolationWarn")
                              : t("form.endpoint.onViolationEndSession")}
                          </span>
                          <p className="m-0 text-xs text-muted-foreground">
                            {opt === "warn" ? t("form.endpoint.warnHint") : t("form.endpoint.endSessionHint")}
                          </p>
                        </div>
                      </label>
                    ))}
                ...
                <div>
                  <label
                    ...
                    className="mb-1 block text-sm font-medium text-foreground"
                  >
                    {t("form.endpoint.violationMessage")}
                  </label>
                  <p className="mb-2 text-xs text-muted-foreground">{t("form.endpoint.violationMessageHint")}</p>
                  <Textarea
  ...
  const getStepConfigs = () => {
    if (shouldRenderContentFilterConfigSettings(selectedProvider)) {
      return [
        { title: t("form.steps.basicInfo"), optional: false },
        { title: t("form.steps.topics"), optional: false },
        { title: t("form.steps.patterns"), optional: false },
        { title: t("form.steps.keywords"), optional: false },
        { title: t("form.steps.endpointSettings"), optional: true },
      ];
    }
    if (shouldRenderPIIConfigSettings(selectedProvider)) {
      return [
        { title: t("form.steps.basicInfo"), optional: false },
        { title: t("form.steps.piiConfiguration"), optional: false },
      ];
    }
    return [
      { title: t("form.steps.basicInfo"), optional: false },
      { title: t("form.steps.providerConfiguration"), optional: false },
    ];
  };
                      ...
                      <div className={`min-w-0 flex-1 ${isLast ? "" : "pb-4"}`}>
                        ...
                        >
                          ...
                          {step.optional && !isCurrent && (
                            <span className="text-[11px] text-muted-foreground">{t("form.steps.optional")}</span>
                          )}
                          {isDone && (
                            <span className="text-[11px] text-info hover:underline">{t("form.steps.edit")}</span>
                          )}
                        </div>
