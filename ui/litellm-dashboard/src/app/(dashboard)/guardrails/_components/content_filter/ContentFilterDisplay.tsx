    ...
    <>
        ...
        <Card className="mt-6">
          <CardContent>
            <div className="mb-4 flex items-center justify-between">
              <p className="text-lg font-semibold">{t("contentFilter.display.contentCategories")}</p>
              <Badge variant="secondary">
                {t("contentFilter.display.categoriesConfigured", { count: categories.length })}
              </Badge>
            </div>
            <CategoryTable
      ...
      {patterns.length > 0 && (
        <Card className="mt-6">
          <CardContent>
            <div className="mb-4 flex items-center justify-between">
              <p className="text-lg font-semibold">{t("contentFilter.config.patternDetection")}</p>
              <Badge variant="secondary">
                {t("contentFilter.display.patternsConfigured", { count: patterns.length })}
              </Badge>
            </div>
            <PatternTable
              patterns={patterns}
      ...
      {blockedWords.length > 0 && (
        <Card className="mt-6">
          <CardContent>
            <div className="mb-4 flex items-center justify-between">
              <p className="text-lg font-semibold">{t("contentFilter.config.blockedKeywords")}</p>
              <Badge variant="secondary">
                {t("contentFilter.display.keywordsConfigured", { count: blockedWords.length })}
              </Badge>
            </div>
            <KeywordTable
