...
const GuardrailTestPlayground: React.FC<GuardrailTestPlaygroundProps> = ({
  ...
  const handleTestGuardrails = async (text: string, metadata?: Record<string, unknown> | null) => {
    ...
    if (results.length > 0) {
      toast.success(
        t(results.length > 1 ? "testPlayground.toast.appliedOther" : "testPlayground.toast.appliedOne", {
    ...
    if (errors.length > 0) {
      toast.fromError(
        t(errors.length > 1 ? "testPlayground.toast.failedOther" : "testPlayground.toast.failedOne", {
          value: errors.length,
        }),
      );
    }
          ...
          <div className="flex h-full">
            ...
            <div className="flex w-1/4 flex-col overflow-hidden border-r border-border">
              <div className="border-b border-border p-4">
                <div className="mb-3">
                  <h3 className="mb-3 text-lg font-semibold">{t("testPlayground.guardrailsHeading")}</h3>
                  <InputGroup>
                    <InputGroupAddon>
              ...
              <div className="flex-1 overflow-auto">
                {isLoading ? (
                  ...
                  <div className="p-4 text-center text-muted-foreground">
                    {searchQuery ? t("testPlayground.noMatch") : t("testPlayground.noAvailable")}
                  </div>
                ) : (
            ...
            <div className="flex w-3/4 flex-col">
              <div className="flex items-center justify-between border-b border-border p-4">
                <h2 className="mb-0 text-xl font-semibold">{t("testPlayground.title")}</h2>
              </div>
              ...
              <div className="flex-1 overflow-auto p-4">
                {selectedGuardrails.size === 0 ? (
                  <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
                    ...
                    <p className="mb-2 text-lg font-medium">{t("testPlayground.emptyTitle")}</p>
                    <p className="max-w-md text-center">{t("testPlayground.emptyDescription")}</p>
                  </div>
                ) : (
