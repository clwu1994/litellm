    ...
    <div className="space-y-3">
      ...
      {availableKeys.length > 0 && (
        <div className="mt-2 flex items-center space-x-3">
          ...
          <span className="text-sm text-muted-foreground">{t("optionalParams.selectCategoryHint")}</span>
        </div>
      )}
    </div>
  );
    ...
    return (
      <div key={fullFieldKey} className="mb-8 rounded-lg border border-border bg-card p-6 shadow-xs">
        <GuardrailField
          ...
          rules={field.required ? requiredRule(t("providerFields.fieldRequired", { field: fieldKey })) : undefined}
  ...
  return (
    <div className="guardrail-optional-params">
      <div className="mb-8 border-b border-border pb-4">
        <h3 className="mb-2 text-lg font-semibold text-foreground">{t("optionalParams.title")}</h3>
        <p className="text-sm text-muted-foreground">
          {optionalParams.description || t("optionalParams.descriptionFallback")}
        </p>
      </div>
