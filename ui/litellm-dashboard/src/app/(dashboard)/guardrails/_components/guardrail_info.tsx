...
const booleanItems = (t: TFunction<"guardrails">) => [
...
const GuardrailInfoView: React.FC<GuardrailInfoProps> = ({ guardrailId, onClose, accessToken, isAdmin }) => {
  ...
  const defaultOnItems = useMemo(() => booleanItems(t), [t]);
  ...
  const fetchGuardrailInfo = async () => {
    try {
      ...
      toast.fromError(t("info.toast.loadFailed"));
  ...
  const handleGuardrailUpdate = async (values: GuardrailFormValues) => {
    try {
      ...
      if (Object.keys(updateData).length === 0) {
        toast.info(t("info.toast.noChanges"));
        setIsEditing(false);
        return;
      }
      ...
      toast.fromError(t("info.toast.updateFailed"));
    }
  };
  ...
  const piiEntityCount = Object.keys(guardrailData.litellm_params?.pii_entities_config ?? {}).length;
  ...
  return (
    <div className="p-4">
      <div>
        {backButton}
        <h1 className="text-2xl font-semibold">{guardrailData.guardrail_name || t("common.unnamedGuardrail")}</h1>
        ...
        <div>
          ...
          <TabsContent value="overview" keepMounted>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <Card className="block p-6">
                ...
                <div className="mt-2">
                  ...
                  <p>{t("info.lastUpdatedWithDate", { date: formatDate(guardrailData.updated_at) })}</p>
                </div>
              </Card>
            </div>
              ...
              Object.keys(guardrailData.litellm_params.pii_entities_config).length > 0 && (
                <Card className="block mt-6 p-6">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge variant="secondary">{t("info.piiEntitiesConfigured", { value: piiEntityCount })}</Badge>
                  </div>
                </Card>
              )}
            ...
            {guardrailData.litellm_params?.pii_entities_config &&
              Object.keys(guardrailData.litellm_params.pii_entities_config).length > 0 && (
                <Card className="block mt-6 p-6">
                  <p className="mb-4 text-lg font-semibold">{t("info.piiEntityConfiguration")}</p>
            ...
            <TabsContent value="settings" keepMounted>
              <Card className="block p-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  {isConfigGuardrail && (
                    <SimpleTooltip content={t("info.configEditHint")}>
                      <Info role="img" aria-label={t("info.configAria")} className="size-4 text-muted-foreground" />
                    </SimpleTooltip>
                  )}
                  {!isEditing &&
                    !isConfigGuardrail &&
                ...
                {isEditing ? (
                    ...
                    <form onSubmit={form.handleSubmit(submitLatest)}>
                      <FieldGroup>
                        <GuardrailField
                          ...
                          name="guardrail_name"
                          label={t("info.guardrailName")}
                          rules={requiredRule(t("info.validation.nameRequired"))}
                        >
                          {({ ref, value, ...field }) => (
                        ...
                        <GuardrailField control={form.control} name="default_on" label={t("info.defaultOn")}>
                        ...
                        <GuardrailField
                          ...
                          label={labelWithHint(t("info.skipSystemMessageLabel"), t("info.skipSystemMessageHint"))}
                        >
                        ...
                        <GuardrailField
                          ...
                          label={labelWithHint(t("info.skipToolMessageLabel"), t("info.skipToolMessageHint"))}
                        >
                          {(fieldControl) => <SkipMessageSelect control={fieldControl} />}
                        </GuardrailField>
                        {guardrailData.litellm_params?.guardrail === "presidio" && (
                          <>
                            <SectionHeading>{t("info.piiProtection")}</SectionHeading>
                            <div className="mb-6">
                              {guardrailSettings && (
                                <PiiConfiguration
                        ...
                        {(guardrailData.litellm_params?.guardrail === "tool_permission" ||
                          ...
                          <SectionHeading>{t("info.providerSettings")}</SectionHeading>
                        )}
                        ...
                        <SectionHeading>{t("info.advancedSettings")}</SectionHeading>
                        <GuardrailField control={form.control} name="guardrail_info" label={t("info.guardrailInfo")}>
                          {({ ref, value, ...field }) => (
                        ...
                        <div className="mt-6 flex justify-end gap-2">
                          ...
                          <Button type="submit">{t("info.saveChanges")}</Button>
                        </div>
                      </FieldGroup>
                    </form>
                  </TooltipProvider>
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="font-medium">{t("info.guardrailId")}</p>
                      <div className="font-mono">{guardrailData.guardrail_id}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("info.guardrailName")}</p>
                      <div>{guardrailData.guardrail_name || t("common.unnamedGuardrail")}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("info.provider")}</p>
                      <div>{displayName}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("info.mode")}</p>
                      <div>{formatGuardrailMode(guardrailData.litellm_params?.mode, t) || "-"}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("info.defaultOn")}</p>
                      <Badge variant={guardrailData.litellm_params?.default_on ? "secondary" : "outline"}>
                        ...
                        <div>
                          ...
                          <div className="mt-2">
                            <Badge variant="secondary">
                              {t("info.piiEntitiesConfigured", { value: piiEntityCount })}
                            </Badge>
                          </div>
                        </div>
                      )}
                    ...
                    <div>
                      <p className="font-medium">{t("info.createdAt")}</p>
                      <div>{formatDate(guardrailData.created_at)}</div>
                    </div>
                    <div>
                      <p className="font-medium">{t("info.lastUpdated")}</p>
