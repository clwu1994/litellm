...
const TestVectorStoreTab: React.FC<TestVectorStoreTabProps> = ({ accessToken, vectorStores }) => {
  ...
  if (!accessToken) {
    return (
      <Card>
        <CardContent>
          <p className="text-sm text-muted-foreground">{t("test.noAccessToken")}</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card>
        <CardContent>
          <div className="py-8 text-center">
            <p className="text-sm text-muted-foreground">{t("test.noStores")}</p>
          </div>
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="space-y-4">
          <div>
            <h5 className="text-base font-medium text-foreground">{t("test.selectStore")}</h5>
            <p className="text-sm text-muted-foreground">{t("test.selectStoreHint")}</p>
          </div>
          ...
          >
            <ComboboxInput className="w-full" placeholder={t("test.selectStorePlaceholder")} />
            <ComboboxContent>
              <ComboboxEmpty>{t("test.noMatchingStores")}</ComboboxEmpty>
              <ComboboxList>
                {(store: VectorStore) => (
