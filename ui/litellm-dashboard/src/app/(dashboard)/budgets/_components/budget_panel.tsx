...
const BudgetPanel: React.FC<BudgetSettingsPageProps> = ({ accessToken }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.fromError(t("toast.deleteError"));
    } finally {
    ...
    <main className="flex h-full flex-col p-8">
      <Tabs defaultValue="budgets" className="min-h-0 flex-1 gap-6">
        ...
        <TabsContent value="budgets" className="flex min-h-0 flex-1 flex-col" keepMounted>
          <div className="flex min-h-0 flex-1 flex-col">
            ...
            <DeleteResourceModal
              ...
              resourceInformationTitle={t("delete.infoTitle")}
              resourceInformation={[
                { label: t("table.header.budgetId"), value: selectedBudget?.budget_id, code: true },
                { label: t("table.header.maxBudget"), value: selectedBudget?.max_budget },
                { label: t("table.header.tpm"), value: selectedBudget?.tpm_limit },
                { label: t("table.header.rpm"), value: selectedBudget?.rpm_limit },
              ]}
        ...
        <TabsContent value="examples" className="min-h-0 flex-1 overflow-y-auto" keepMounted>
          <div className="pt-6">
            <p className="text-base text-muted-foreground">{t("examples.heading")}</p>
