...
const CacheLeakageCard: React.FC<CacheLeakageCardProps> = ({ activity }) => {
  ...
  const subject = dimension === "model" ? t("cacheLeakage.subjectModel") : t("cacheLeakage.subjectKey");
  const firstColumn = dimension === "model" ? t("cacheLeakage.columnModel") : t("cacheLeakage.columnKey");
  const emptyNoun = dimension === "model" ? t("cacheLeakage.emptyNounModel") : t("cacheLeakage.emptyNounKey");
        ...
        <CardHeader>
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0">
              <CardTitle>{dimension === "model" ? t("cacheLeakage.titleModel") : t("cacheLeakage.titleKey")}</CardTitle>
          ...
          <Tabs value={dimension} onValueChange={(value) => setDimension(value === "model" ? "model" : "key")}>
            <TabsList>
              <TabsTrigger value="key">{t("cacheLeakage.tabs.byKey")}</TabsTrigger>
              <TabsTrigger value="model">{t("cacheLeakage.tabs.byModel")}</TabsTrigger>
            </TabsList>
          </Tabs>
        </CardHeader>
        <CardContent>
          {rows.length > 0 && isFetchingMore && (
            <p className="mb-2 text-sm text-muted-foreground">{t("cacheLeakage.streaming")}</p>
          )}
          {rows.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">
              {loading || isFetchingMore ? t("common.loading") : t("cacheLeakage.empty", { noun: emptyNoun })}
            </p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <SortableHead
                    ...
                    label={t("cacheLeakage.column.uncachedInputTokens")}
                    info={t("cacheLeakage.info.uncachedInputTokens")}
                    sort={sort}
                    onSort={onSort}
                  />
                  <SortableHead
                    column="cacheHitRatio"
                    label={t("cacheLeakage.column.cacheHitRate")}
                    info={t("cacheLeakage.info.cacheHitRate")}
                    sort={sort}
                    onSort={onSort}
                  />
                  <SortableHead
                    column="potentialSavings"
                    label={t("cacheLeakage.column.potentialSavings")}
                    info={t("cacheLeakage.info.potentialSavings")}
                    sort={sort}
                    onSort={onSort}
                  />
                </TableRow>
