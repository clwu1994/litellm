...
const HeroCard: React.FC<{ view: BenchmarkView }> = ({ view }) => {
  ...
  const classificationRate =
    stats.classifier_cost == null ? null : classificationRatePer1kTurns(stats.classifier_cost, stats.turns);
  return (
    <Card className="overflow-hidden py-0">
      <div className="grid md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className="flex flex-col items-center justify-center gap-2 p-6">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            {t("benchmarks.totalSavings")}
          </p>
        ...
        <div className="flex flex-col justify-center border-t p-6 md:border-t-0 md:border-l">
          <SpendRow label={t("benchmarks.actualSpend")} value={usd(stats.spend)} />
          <div className="mb-3 border-l-2 pl-4">
            <SpendRow
              subdued
              label={t("benchmarks.llmSpend")}
              value={
                stats.classifier_cost == null ? t("benchmarks.unavailable") : usd(stats.spend - stats.classifier_cost)
              }
            />
            <SpendRow
              subdued
              label={t("benchmarks.classificationCost")}
              value={stats.classifier_cost == null ? t("benchmarks.unavailable") : usd(stats.classifier_cost)}
              hint={classificationRate ? t(classificationRate.key, { value: classificationRate.value }) : undefined}
            />
          </div>
          {stats.classifier_cost == null && (
            <p className="mb-3 text-xs text-muted-foreground">{t("benchmarks.breakdownUnavailable")}</p>
          )}
          <Separator />
          <SpendRow label={t("benchmarks.baselineSpend")} value={usd(stats.baseline_spend)} />
        </div>
      </div>
    </Card>
  );
  ...
  return (
    <div className="flex flex-col gap-1">
      <div
        ...
        role="img"
        aria-label={t("benchmarks.cache.shareOfTurnsAria")}
      >
        {segments.map((b) => (
          <div
            key={b.key}
            ...
            title={t("benchmarks.cache.bucketTitle", { label: t(b.labelKey), turns: b.turns.toLocaleString() })}
          />
        ))}
      </div>
    ...
    <Table className="border-b">
      <TableHeader>
        <TableRow className="hover:bg-transparent">
          <TableHead className="text-[11px] uppercase tracking-wide">{t("benchmarks.cache.bucket")}</TableHead>
          <TableHead className="text-right text-[11px] uppercase tracking-wide">
            {t("benchmarks.cache.turns")}
          </TableHead>
          <TableHead className="w-1/2" />
          <TableHead className="text-right text-[11px] uppercase tracking-wide">
            {t("benchmarks.cache.hitRate")}
          </TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
          ...
          <TableRow key={b.key} className="hover:bg-transparent">
            <TableCell className="text-foreground">
              <span className="flex items-center gap-2">
                ...
                <span>
                  {t(b.labelKey)}
                  <span className="block text-xs font-normal text-muted-foreground">{t(b.sublabelKey)}</span>
                </span>
              </span>
            </TableCell>
      ...
      <div className="grid lg:grid-cols-[1fr_3fr]">
        <div className="flex flex-col border-b p-6 lg:border-b-0 lg:border-r">
          <div className="flex flex-1 flex-col justify-center gap-3">
            <p className="text-sm text-muted-foreground">{t("benchmarks.cache.cacheHitRate")}</p>
              ...
              <Tooltip>
                ...
                >
                  <span className="text-sm text-muted-foreground underline decoration-dotted underline-offset-2">
                    {t("benchmarks.cache.expiredMiss")}
                  </span>
                  <span className="font-medium tabular-nums text-foreground">{pctLabel(expiredMissPct)}</span>
                </TooltipTrigger>
                <TooltipContent className="max-w-64">{t("benchmarks.cache.expiredMissHint")}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
        </div>
        ...
        <div className="flex flex-col gap-3 p-6">
          <div className="flex items-baseline justify-between">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
              {t("benchmarks.cache.shareOfTurns")}
            </p>
            <p className="text-xs text-muted-foreground">
              <Trans
                ns="costTracking"
                i18nKey="benchmarks.cache.turnsMeasured"
                values={{ turns: total.toLocaleString() }}
                components={{ amount: <span className="text-lg font-semibold tabular-nums text-foreground" /> }}
              />
            </p>
          </div>
          ...
          {cache.unordered_turns > 0 && (
            <p className="text-xs text-muted-foreground">
              {t("benchmarks.cache.unordered", { turns: cache.unordered_turns.toLocaleString() })}
            </p>
          )}
        </div>
      </div>
...
const BenchmarksBody: React.FC<BenchmarksBodyProps> = ({ isPending, error, data, selectedKey, autoRouters }) => {
  const { t } = useTranslation("costTracking");
  if (isPending) return <Message>{t("benchmarks.loading")}</Message>;
  if (error instanceof ApiError && error.status === 403) {
    return <Message>{t("benchmarks.adminOnly")}</Message>;
  }
  if (error || !data) return <Message>{t("benchmarks.unavailableNow")}</Message>;

  const view = viewFor(data, selectedKey);
  const stats = view.stats;
  const avgSessionLength = durationParts(stats.avg_session_seconds);
  return (
    <>
      <HeroCard view={view} />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Metric
          label={t("benchmarks.avgSavedPerSession")}
          value={usd(stats.saved_per_session)}
          hint={t("benchmarks.sessionsHint", { sessions: stats.sessions.toLocaleString() })}
        />
        <Metric label={t("benchmarks.avgTurnsPerSession")} value={stats.avg_turns_per_session.toFixed(1)} />
        <Metric
          label={t("benchmarks.avgSessionLength")}
          value={t(avgSessionLength.unitKey, { value: avgSessionLength.value })}
        />
        <Metric
          label={t("benchmarks.avgTokensPerSession")}
          value={formatNumberWithCommas(stats.avg_tokens_per_session, 1, true)}
        />
      </div>

      <p className="text-xs text-muted-foreground">{t("benchmarks.explanation")}</p>

      <div className="space-y-4">
        <div className="flex flex-wrap items-baseline gap-2">
          <h3 className="text-lg font-semibold text-foreground">{t("benchmarks.cacheTitle")}</h3>
          <p className="text-xs text-muted-foreground">{t("benchmarks.cacheSubtitle")}</p>
        </div>
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">{t("benchmarks.title")}</h2>
          {rangeLabel && (
            <p className="mt-1 text-sm text-muted-foreground">{t("benchmarks.range", { range: rangeLabel })}</p>
          )}
        </div>
        <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
          ...
          <div className="w-full sm:w-64">
            <Select value={selectedKey} onValueChange={(value: string | null) => setSelectedKey(value ?? ALL_ROUTERS)}>
              ...
              <SelectContent>
                <SelectItem value={ALL_ROUTERS}>{t("benchmarks.allRouters")}</SelectItem>
                {groups.map((g) => (
    ...
    <Tabs defaultValue="usage" onValueChange={handleTabChange} className="w-full gap-4">
      <TabsList>
        <TabsTrigger value="usage" className="px-3">
          {t("benchmarks.tabs.usage")}
        </TabsTrigger>
        <TabsTrigger value="shadow-evals" className="px-3">
          {t("benchmarks.tabs.shadowEvals")}
        </TabsTrigger>
      </TabsList>
