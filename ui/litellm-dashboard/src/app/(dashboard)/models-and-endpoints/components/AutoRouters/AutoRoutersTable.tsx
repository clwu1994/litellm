...
function EmptyState({ canModify }: { canModify: boolean }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("autoRouters.table.noRouters")}</div>
      <div className="text-sm text-muted-foreground">
        {canModify ? t("autoRouters.table.emptyCreate") : t("autoRouters.table.emptyReadOnly")}
      </div>
    </div>
  );
}
...
export function AutoRoutersTable({
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      loadingMessage={t("autoRouters.table.loading")}
