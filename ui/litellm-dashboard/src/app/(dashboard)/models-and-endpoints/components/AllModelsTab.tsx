...
const AllModelsTab = ({
  ...
  const teamOptions = useMemo(
    () => [
      { value: PERSONAL_TEAM_VALUE, label: t("table.personalTeam") },
      ...(teams ?? [])
  ...
  const handleTogglePause = useCallback(
    async (modelId: string, blocked: boolean) => {
      ...
      try {
        ...
        toast.success(blocked ? t("toast.modelPaused") : t("toast.modelResumed"));
          ...
          <div className="flex items-start gap-2 px-1 text-xs text-muted-foreground">
            ...
            {selectedTeamValue === PERSONAL_TEAM_VALUE ? (
              <span>
                <Trans
                  ns="models"
                  i18nKey="hint.personal"
                  components={{
                    keysPage: <a href={uiHref("api-keys")} className="font-medium text-info hover:underline" />,
                  }}
                />
              </span>
            ) : (
            ...
            ? [
                {
                  ...
                  value: modelToDelete.model_name || t("delete.notSet"),
                },
                {
                  label: t("delete.litellmModelName"),
                  value: modelToDelete.litellm_model_name || t("delete.notSet"),
                },
                {
                  label: t("delete.provider"),
                  value: modelToDelete.provider || t("delete.notSet"),
                },
                {
                  label: t("delete.createdBy"),
                  value: modelToDelete.model_info?.created_by || t("delete.notSet"),
                },
              ]
            : []
        }
