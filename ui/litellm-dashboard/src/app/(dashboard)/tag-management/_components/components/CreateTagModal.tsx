...
const createTagShape = (t: TFunction<"tagManagement">) => ({
  tag_name: z.string().min(1, t("validation.tagNameRequired")),
...
const createTagSchema = (t: TFunction<"tagManagement">) => z.object(createTagShape(t));
    ...
    <Dialog open={visible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        ...
        <form onSubmit={form.handleSubmit(handleFinish)} noValidate>
          <TooltipProvider>
            <FieldGroup>
              <FormField control={form.control} name="tag_name" label={t("createModal.tagName")}>
                {({ ref, ...field }) => <Input {...field} ref={ref} />}
              </FormField>

              <FormField control={form.control} name="description" label={t("createModal.description")}>
              ...
              <FormField
                ...
                name="allowed_llms"
                label={labelWithHint(t("createModal.allowedModels"), t("createModal.allowedModelsHint"))}
              >
                {({ value, onChange }) => (
                  <MultiSelect
                    ...
                    placeholder={t("createModal.selectModels")}
                  />
                )}
              </FormField>
            </FieldGroup>
              ...
              <CollapsibleContent className="px-4 pb-4">
                <FieldGroup className="mt-4">
                  <FormField
                    control={form.control}
                    name="max_budget"
                    label={labelWithHint(t("createModal.maxBudget"), t("createModal.maxBudgetHint"))}
                  >
                  ...
                  <FormField
                    ...
                    label={labelWithHint(t("createModal.resetBudget"), t("createModal.resetBudgetHint"))}
                  >
                    {({ id, value, onChange }) => (
                ...
                <div className="mt-4 rounded-md border border-border bg-muted p-3">
                  <p className="text-sm text-muted-foreground">
                    <Trans
                      ns="tagManagement"
                      i18nKey="createModal.tpmRpmUnsupported"
                      components={{
                        "1": (
                          <a
