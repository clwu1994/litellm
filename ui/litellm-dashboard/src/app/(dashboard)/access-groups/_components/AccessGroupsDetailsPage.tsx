...
export function AccessGroupDetail({ accessGroupId, onBack }: AccessGroupDetailProps) {
  ...
  return (
    <div className="p-6 px-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 text-sm text-muted-foreground">
              <span>{t("details.idLabel", { id: accessGroup.access_group_id })}</span>
              <CopyButton value={accessGroup.access_group_id} label={t("details.copyIdAria")} />
            </div>
          </div>
        </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>{t("details.groupDetails")}</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <Card>
            ...
            {keys.length > MAX_PREVIEW && (
              <CardAction>
                <Button variant="link" size="sm" onClick={() => setShowAllKeys(!showAllKeys)}>
                  {showAllKeys ? t("details.showLess") : t("details.viewAll", { total: keys.length })}
                </Button>
              </CardAction>
            )}
          </CardHeader>
          <CardContent>
            {keys.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground">{t("details.noKeys")}</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
            ...
            {teams.length > MAX_PREVIEW && (
              <CardAction>
                <Button variant="link" size="sm" onClick={() => setShowAllTeams(!showAllTeams)}>
                  {showAllTeams ? t("details.showLess") : t("details.viewAll", { total: teams.length })}
                </Button>
              </CardAction>
            )}
          </CardHeader>
          <CardContent>
            {teams.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground">{t("details.noTeams")}</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <Tabs defaultValue="models">
            ...
            <TabsContent value="models" className="pt-4">
              <ResourceList items={models} emptyMessage={t("details.noModels")} />
            </TabsContent>
            <TabsContent value="mcp" className="pt-4">
              <ResourceList items={mcpServers} emptyMessage={t("details.noMcpServers")} />
            </TabsContent>
            <TabsContent value="agents" className="pt-4">
              <ResourceList items={agents} emptyMessage={t("details.noAgents")} />
            </TabsContent>
          </Tabs>
        </CardContent>
      </Card>
