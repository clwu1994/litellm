...
const TransformRequestPanel: React.FC<TransformRequestPanelProps> = ({ accessToken }) => {
  const { t } = useTranslation("transformRequest");
...
${formattedBody}
  ...
  const handleTransform = async () => {
    ...
    try {
      ...
      try {
        ...
        toast.fromError(t("toast.invalidJson"));
        setIsLoading(false);
        return;
      }
      ...
      if (!accessToken) {
        toast.fromError(t("toast.noAccessToken"));
        setIsLoading(false);
        return;
      }
      ...
      if (data.raw_request_api_base && data.raw_request_body) {
        ...
        toast.success(t("toast.transformed"));
      } else {
        ...
        toast.info(t("toast.unexpectedFormat"));
      }
    } catch (err) {
      console.error("Error transforming request:", err);
      toast.fromError(t("toast.transformFailed"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="p-2">
      <h1 className="text-lg font-medium text-foreground">{t("header.title")}</h1>
      <p className="text-sm text-muted-foreground">{t("header.subtitle")}</p>
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        {/* Original Request Panel */}
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl font-bold">{t("originalRequest.title")}</CardTitle>
            <CardDescription>{t("originalRequest.description")}</CardDescription>
          </CardHeader>
          ...
          <CardContent>
            <Textarea
              ...
              placeholder={t("originalRequest.placeholder")}
            />
          </CardContent>

          <CardFooter className="justify-end">
            <Button onClick={handleTransform} disabled={isLoading}>
              <span>{t("originalRequest.submit")}</span>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl font-bold">{t("transformedRequest.title")}</CardTitle>
            <CardDescription>{t("transformedRequest.description")}</CardDescription>
            <p className="mt-2 text-xs text-muted-foreground">{t("transformedRequest.note")}</p>
          </CardHeader>
  ...
  "temperature": 0.7
              ...
              <Button
                variant="ghost"
                size="icon-sm"
                aria-label={t("transformedRequest.copy")}
