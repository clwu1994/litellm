    ...
    >
      <SheetContent className="overflow-y-auto data-[side=right]:w-full data-[side=right]:max-w-full data-[side=right]:sm:w-[720px] data-[side=right]:sm:max-w-full">
        <SheetHeader className="border-b">
          <SheetTitle>{row ? <code className={CODE_CLASS}>{row.key}</code> : t("page.title")}</SheetTitle>
        </SheetHeader>
        {row && (
          <div className="flex flex-col gap-4 px-4 pb-4">
            <div className="flex flex-wrap gap-x-8 gap-y-3">
              <div>
                <span className={`block ${LABEL_CLASS}`}>{t("drawer.memoryId")}</span>
                <code className={CODE_CLASS}>{row.memory_id}</code>
              </div>
              <div>
                <span className={`block ${LABEL_CLASS}`}>{t("drawer.userId")}</span>
                <span className={row.user_id ? "text-sm text-foreground" : "text-sm text-muted-foreground"}>
                  {row.user_id ?? "-"}
                </span>
              </div>
              <div>
                <span className={`block ${LABEL_CLASS}`}>{t("drawer.teamId")}</span>
            ...
            <div>
              <span className={LABEL_CLASS}>{t("drawer.value")}</span>
              <p className={`${BLOCK_CLASS} text-[13px]`}>{row.value}</p>
            </div>
            {row.metadata !== undefined && row.metadata !== null && (
              <div>
                <span className={LABEL_CLASS}>{t("drawer.metadata")}</span>
                <p className={`${BLOCK_CLASS} text-xs`}>{JSON.stringify(row.metadata, null, 2)}</p>
              </div>
            )}
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              <span>
                {t("drawer.created", { timestamp: formatTimestamp(row.created_at) })}
                {row.created_by ? t("drawer.by", { name: row.created_by }) : ""}
              </span>
              <span aria-hidden="true">·</span>
              <span>
                {t("drawer.updated", { timestamp: formatTimestamp(row.updated_at) })}
                {row.updated_by ? t("drawer.by", { name: row.updated_by }) : ""}
              </span>
            </div>
          </div>
        )}
