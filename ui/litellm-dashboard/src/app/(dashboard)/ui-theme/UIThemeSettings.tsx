...
const UIThemeSettings: React.FC<UIThemeSettingsProps> = ({ userID, userRole, accessToken }) => {
  const { t } = useTranslation("uiTheme");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.fromError(t("toast.saveFailed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      toast.fromError(t("toast.resetFailed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full mx-auto max-w-4xl px-6 py-8">
      <div className="mb-8">
        <h1 className="mb-2 text-2xl font-bold">{t("header.title")}</h1>
      ...
      <Card>
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-muted-foreground">{t("logoDark.hint")}</p>
          </div>
          <div>
            ...
            />
            <p className="mt-1 text-xs text-muted-foreground">{t("favicon.hint")}</p>
          </div>
