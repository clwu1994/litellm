  ...
  const handleSubmit = async (formValues: any) => {
    ...
    try {
      ...
      if (!hasUserUpdates && !hasTeamAdditions) {
        toast.fromError(t("bulk.toast.nothingToUpdate"));
        return;
      }
      ...
      if (hasUserUpdates) {
        if (updateAllUsers) {
          ...
          successMessages.push(t("bulk.toast.updatedAll", { count: result.total_requested }));
        } else {
          await userBulkUpdateUserCall(accessToken, updatePayload, userIds);
          successMessages.push(t("bulk.toast.updatedUsers", { count: userIds.length }));
        }
      }
      ...
      if (hasTeamAdditions) {
        ...
        if (successfulTeams.length > 0) {
          ...
          successMessages.push(
            t("bulk.toast.addedToTeams", { teams: successfulTeams.length, additions: totalAdditions }),
          );
        }

        if (failedTeams.length > 0) {
          toast.warning(t("bulk.toast.addToTeamsFailed", { count: failedTeams.length }));
        }
      }
      ...
      toast.fromError(t("bulk.toast.operationFailed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        <DialogHeader>
          <DialogTitle>
            {updateAllUsers ? t("bulk.title.all") : t("bulk.title.selected", { count: selectedUsers.length })}
          </DialogTitle>
        </DialogHeader>
        {allowAllUsers && (
          <div className="mb-4">
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                aria-label={t("bulk.updateAllUsers")}
              />
              <label htmlFor={updateAllUsersId} className="cursor-pointer text-sm font-medium text-foreground">
                {t("bulk.updateAllUsers")}
              </label>
            </div>
            {updateAllUsers && (
              <div className="mt-2">
                <span className="text-xs text-warning">{t("bulk.warningAllUsers")}</span>
              </div>
            )}
          </div>
        )}
        ...
        {!updateAllUsers && (
          <div className="mb-4">
            <h5 className="mb-2 text-sm font-semibold text-foreground">
              {t("bulk.selectedUsers", { count: selectedUsers.length })}
            </h5>
            <div className="max-h-[200px] overflow-y-auto rounded-md border border-border">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[30%]">{t("bulk.table.userId")}</TableHead>
                    <TableHead className="w-[25%]">{t("bulk.table.email")}</TableHead>
                    <TableHead className="w-[25%]">{t("bulk.table.currentRole")}</TableHead>
                    <TableHead className="w-[20%]">{t("bulk.table.budget")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {selectedUsers.map((user) => (
                    <TableRow key={user.user_id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {user.user_email || t("bulk.value.noEmail")}
                      </TableCell>
        ...
        <div className="mb-4">
          <p className="text-sm text-foreground">
            <Trans ns="users" i18nKey="bulk.instructions" components={{ strong: <strong /> }} />
          </p>
        </div>
        ...
        <Card size="sm" className="mb-4 bg-muted/50">
          <CardHeader>
            <CardTitle>{t("bulk.teamManagement")}</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <>
                  <div>
                    <label htmlFor={selectedTeamsId} className="block text-sm font-medium text-foreground">
                      {t("bulk.field.selectTeams")}
                    </label>
                    <MultiSelect
                      id={selectedTeamsId}
                      className="mt-2"
                      placeholder={t("bulk.placeholder.selectTeams")}
                      value={selectedTeams}
                  ...
                  <div>
                    <label htmlFor={teamBudgetId} className="block text-sm font-medium text-foreground">
                      {t("bulk.field.teamBudget")}
                    </label>
                    <NumericalInput
                      id={teamBudgetId}
                      className="mt-2"
                      placeholder={t("bulk.placeholder.teamBudget")}
                      value={teamBudget ?? ""}
                      ...
                      step={0.01}
                    />
                    <span className="text-xs text-muted-foreground">{t("bulk.hint.teamBudget")}</span>
                  </div>

                  <span className="text-xs text-muted-foreground">{t("bulk.hint.roleDefault")}</span>
                </>
              )}
            </div>
          </CardContent>
        ...
        {loading && (
          <div className="mt-2.5 text-center">
            <span className="text-sm text-foreground">
              {updateAllUsers ? t("bulk.updating.all") : t("bulk.updating.selected", { count: selectedUsers.length })}
            </span>
          </div>
        )}
      </DialogContent>
