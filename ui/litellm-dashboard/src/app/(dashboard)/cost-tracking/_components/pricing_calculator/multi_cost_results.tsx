...
const SingleModelBreakdown: React.FC<{
  ...
  const periodKey = timePeriod === "day" ? "common.daily" : "common.monthly";
    ...
    <div className="space-y-3 bg-muted p-4 rounded-lg">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("calculator.totalPerRequestShort")}</p>
          <p className="text-base font-semibold text-info break-words">{formatCost(result.cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("common.inputCost")}</p>
          <p className="text-sm break-words">{formatCost(result.input_cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("common.outputCost")}</p>
          <p className="text-sm break-words">{formatCost(result.output_cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("common.marginFee")}</p>
      ...
      {periodCost !== null && (
        <div className="grid grid-cols-4 gap-4 pt-2 border-t border-border">
          ...
          <div className="min-w-0">
            <p className="text-xs text-muted-foreground block">
              {t("calculator.periodInput", { period: t(periodKey) })}
            </p>
            <p className="text-sm break-words">{formatCost(periodInputCost)}</p>
          </div>
          <div className="min-w-0">
            <p className="text-xs text-muted-foreground block">
              {t("calculator.periodOutput", { period: t(periodKey) })}
            </p>
            <p className="text-sm break-words">{formatCost(periodOutputCost)}</p>
          </div>
          <div className="min-w-0">
            <p className="text-xs text-muted-foreground block">
              {t("calculator.periodMarginFee", { period: t(periodKey) })}
            </p>
      ...
      {(result.input_cost_per_token || result.output_cost_per_token) && (
        <div className="text-xs text-muted-foreground pt-2 border-t border-border">
          {t("calculator.tokenPricing")}{" "}
          {result.input_cost_per_token && (
            <span>
              {t("calculator.tokenPriceInput", {
  ...
  if (!hasAnyResult && !isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center border border-dashed border-border rounded-lg bg-muted">
        <p className="text-muted-foreground">{t("calculator.selectModelsPrompt")}</p>
      </div>
    );
  }
  ...
  if (!hasAnyResult && isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center">
        ...
        <p className="text-muted-foreground block mt-2">{t("calculator.calculating")}</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-base font-semibold text-foreground">{t("calculator.costEstimates")}</p>
        ...
        {errorEntries.map((e) => (
          ...
          >
            <span className="font-medium">{e.entry.model || t("calculator.unknownModel")}: </span>
            {e.error}
          </div>
        ))}
      </div>
    ...
    <div className="space-y-4">
      ...
      <Card size="sm" className="px-4 bg-linear-to-r from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-950">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
          <div className="min-w-0">
            <span className="text-xs text-muted-foreground">{t("common.totalPerRequest")}</span>
          ...
          <div className="min-w-0">
            <span className="text-xs text-muted-foreground">
              {t("calculator.totalPeriod", { period: t(periodKey) })}
            </span>
            <div
        ...
        {hasMargin && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 mt-3 pt-3 border-t border-border">
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground">{t("calculator.marginFeePerRequest")}</div>
        ...
        <Table className="border border-border rounded-lg">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t("common.perRequest")}</TableHead>
              <TableHead className="text-right">{t("common.marginFee")}</TableHead>
              <TableHead className="text-right">{t(periodKey)}</TableHead>
              <TableHead className="w-10">
                <span className="sr-only">{t("calculator.costBreakdown")}</span>
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
                    ...
                    <TableCell className="text-right">
                      {!record.error && (
                        <Button
                          ...
                          aria-label={t(isExpanded ? "calculator.hideBreakdown" : "calculator.showBreakdown", {
                            model: record.model,
                          })}
