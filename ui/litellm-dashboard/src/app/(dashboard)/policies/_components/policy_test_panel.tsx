...
const ContextCombobox: React.FC<ContextComboboxProps> = ({ id, value, onChange, placeholder, options }) => {
  ...
  return (
    ...
    >
      ...
      <ComboboxContent>
        <ComboboxEmpty>{t("simulator.noOptions")}</ComboboxEmpty>
        <ComboboxList>
          {(item: string) => (
  ...
  return (
    <div>
      <div className="bg-card border border-border rounded-lg p-6 mb-6">
        <div className="mb-5">
          <h3 className="text-base font-semibold mb-1">{t("tabs.simulator")}</h3>
          <span className="text-muted-foreground">{t("simulator.description")}</span>
        </div>

        <form onSubmit={(event) => event.preventDefault()} noValidate>
          <FieldGroup className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="team_alias" label={t("simulator.teamAlias")}>
              {({ id, value, onChange }) => (
                <ContextCombobox
                  id={id}
                  value={value}
                  onChange={onChange}
                  placeholder={t("simulator.teamAliasPlaceholder")}
                  options={availableTeams}
                />
              )}
            </FormField>
            <FormField control={form.control} name="key_alias" label={t("simulator.keyAlias")}>
              {({ id, value, onChange }) => (
                <ContextCombobox
                  id={id}
                  value={value}
                  onChange={onChange}
                  placeholder={t("simulator.keyAliasPlaceholder")}
                  options={availableKeys}
                />
              )}
            </FormField>
            <FormField control={form.control} name="model" label={t("simulator.model")}>
              {({ id, value, onChange }) => (
                <ContextCombobox
                  id={id}
                  value={value}
                  onChange={onChange}
                  placeholder={t("simulator.modelPlaceholder")}
                  options={availableModels}
                />
              )}
            </FormField>
            <FormField control={form.control} name="tags" label={t("simulator.tags")}>
              {({ id, value, onChange, onBlur }) => (
                <TokenSelect
                  ...
                  onBlur={onBlur}
                  placeholder={t("simulator.tagsPlaceholder")}
                  allowCustomValues
                  tokenSeparators={[",", " "]}
                />
              )}
      ...
      {!hasSearched && (
        <div className="bg-card border border-border rounded-lg p-8 text-center">
          ...
          <p className="text-sm font-medium text-foreground mb-1">{t("simulator.emptyTitle")}</p>
          <p className="text-xs text-muted-foreground">{t("simulator.emptyHint")}</p>
        </div>
      )}
        ...
        <div className="bg-card border border-border rounded-lg p-6">
          {result.matched_policies.length === 0 ? (
            <div className="py-6 text-center">
              ...
              <p className="text-sm text-muted-foreground">{t("simulator.noMatch")}</p>
            </div>
          ) : (
            <>
              <div className="mb-4">
                <p className="text-sm font-semibold mb-2">{t("simulator.effectiveGuardrails")}</p>
                <div className="flex flex-wrap gap-1">
                  {result.effective_guardrails.length > 0 ? (
                    ...
                    <span className="text-muted-foreground text-sm">{t("info.none")}</span>
                  )}
                </div>
              </div>

              <div>
                <p className="text-sm font-semibold mb-2">{t("simulator.matchedPolicies")}</p>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-2 pr-4">{t("simulator.columnPolicy")}</th>
                      <th className="text-left py-2 pr-4">{t("simulator.columnMatchedVia")}</th>
                      <th className="text-left py-2">{t("simulator.columnGuardrailsAdded")}</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={p.policy_name} className="border-b border-border last:border-0">
                        ...
                        <td className="py-2">
                          {p.guardrails_added.length > 0 ? (
                            ...
                            <span className="text-muted-foreground">{t("info.none")}</span>
                          )}
                        </td>
                      </tr>
                    ))}
      ...
      {hasSearched && !result && !isLoading && (
        <Alert variant="error">
          <CircleAlert />
          <AlertTitle>{t("simulator.errorTitle")}</AlertTitle>
          <AlertDescription>{t("simulator.errorDescription")}</AlertDescription>
        </Alert>
      )}
    </div>
  );
