...
const PolicyTemplateCard: React.FC<PolicyTemplateCardProps> = ({
  ...
  const complexityKey = COMPLEXITY_KEYS[complexity];
  const complexityLabel = complexityKey === undefined ? complexity : t(complexityKey);
    ...
    <Card className="h-full transition-shadow hover:shadow-md">
      <CardContent className="flex h-full flex-col">
        <div className="mb-4 flex items-start justify-between">
          ...
          <Badge variant="outline">{t("templates.complexityLabel", { level: complexityLabel })}</Badge>
        </div>
        ...
        {inherits && (
          <div className="mb-4 text-xs">
            <span className="text-muted-foreground">{t("templates.inheritsFrom")}</span>
        ...
        <div className="mb-6">
          <span className="mb-2 block text-xs font-medium tracking-wider text-muted-foreground uppercase">
            {t("templates.includedGuardrails")}
          </span>
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        ...
        toast.error(t("templates.toast.fetchFailed"));
      } finally {
        setIsLoading(false);
      }
    };
          ...
          <div className="w-52 shrink-0">
            <div className="sticky top-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold">{t("templates.categories")}</span>
                {selectedTags.size > 0 && (
        ...
        <div className="flex-1">
          {selectedTags.size > 0 && (
            <div className="mb-4 text-sm text-muted-foreground">
              {t("templates.showing", { shown: filteredTemplates.length, total: templates.length })}
            </div>
          )}
