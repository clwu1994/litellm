...
const GuardrailSelectionModal: React.FC<GuardrailSelectionModalProps> = ({
  ...
  const { t: tGuardrails } = useTranslation("guardrails");
  ...
  const guardrailsInfo: GuardrailInfo[] = (template?.guardrailDefinitions || []).map((def: any) => ({
    ...
    description: def.guardrail_info?.description || t("guardrailSelection.noDescription"),
      ...
      <DialogContent className="sm:max-w-175">
          ...
          <DialogTitle className="flex items-center gap-2 text-lg">
            ...
            {progressInfo && (
              <Badge variant="secondary">
                {t("guardrailSelection.progressBadge", { current: progressInfo.current, total: progressInfo.total })}
              </Badge>
            )}
          </DialogTitle>
          <DialogDescription>{t("guardrailSelection.reviewDescription")}</DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4">
            ...
            <div className="flex-1">
              <div className="text-sm">
                <span className="font-medium">
                  {t("guardrailSelection.totalGuardrails", { guardrailCount: guardrailsInfo.length })}
                </span>
                <span className="mx-2 text-muted-foreground">•</span>
                <span className="font-medium text-success">
                  {t("guardrailSelection.newGuardrails", { guardrailCount: newGuardrailsCount })}
                </span>
                {existingCount > 0 && (
                  <>
                    <span className="mx-2 text-muted-foreground">•</span>
                    <span className="text-muted-foreground">
                      {t("guardrailSelection.alreadyExist", { guardrailCount: existingCount })}
                    </span>
                  </>
                )}
              </div>
            ...
            {newGuardrailsCount > 0 && (
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={handleSelectAll}>
                  {t("guardrailSelection.selectAllNew")}
                </Button>
                <Button variant="outline" size="sm" onClick={handleDeselectAll}>
                  {t("guardrailSelection.deselectAll")}
                </Button>
              </div>
            )}
          </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {guardrail.alreadyExists && (
                        <Badge variant="secondary">{t("guardrailSelection.alreadyExistsBadge")}</Badge>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      <Badge variant="outline">
                        {guardrail.definition?.litellm_params?.guardrail || t("guardrailSelection.unknown")}
                      </Badge>
                      <Badge variant="secondary">
                        {formatGuardrailMode(guardrail.definition?.litellm_params?.mode, tGuardrails) ||
                          t("guardrailSelection.unknown")}
                      </Badge>
                      {guardrail.definition?.litellm_params?.patterns && (
                        <Badge variant="secondary">
                          {t("guardrailSelection.patternCount", {
                            patternCount: guardrail.definition.litellm_params.patterns.length,
                          })}
                        </Badge>
                      )}
                      {guardrail.definition?.litellm_params?.categories && (
                        <Badge variant="secondary">
                          {t("guardrailSelection.categoryCount", {
                            categoryCount: guardrail.definition.litellm_params.categories.length,
                          })}
                        </Badge>
                      )}
                    </div>
          ...
          {guardrailsInfo.length === 0 && (
            <div className="py-8 text-center text-muted-foreground">
              <p>{t("guardrailSelection.noneDefined")}</p>
              <p className="text-sm mt-2">{t("guardrailSelection.usesExisting")}</p>
            </div>
          )}
              ...
              <div className="rounded-lg border border-border bg-muted p-3">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    {t("guardrailSelection.discoveredCompetitors", {
                      competitorCount: template.discoveredCompetitors.length,
                    })}
                  </span>
                </div>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {t("guardrailSelection.discoveredCompetitorsHint")}
                </p>
              </div>
            </>
          )}
          ...
          <div className="text-sm text-muted-foreground">
            {selectedCount > 0 ? (
              <p>
                ...
                {t(
                  selectedCount > 1
                    ? "guardrailSelection.guardrailsWillBeCreated"
                    : "guardrailSelection.guardrailWillBeCreated",
                )}
              </p>
            ) : existingCount > 0 ? (
              <p className="text-success">{t("guardrailSelection.allExist")}</p>
            ) : (
              <p className="text-warning">{t("guardrailSelection.selectAtLeastOne")}</p>
            )}
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={isLoading || (selectedCount === 0 && existingCount === 0)}>
            {selectedCount > 0
              ? t(selectedCount > 1 ? "guardrailSelection.createAndUse" : "guardrailSelection.createAndUseSingular", {
                  guardrailCount: selectedCount,
                })
