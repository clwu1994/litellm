...
export function ToolTestPanel({
  ...
  const handleCopyResult = async () => {
    ...
    if (success) {
      toast.success(t("toolTest.copyResultSuccess"));
    } else {
      toast.fromError(t("toolTest.copyResultFailed"));
    }
  };
  ...
  const handleCopyToolName = async () => {
    ...
    if (success) {
      toast.success(t("toolTest.copyNameSuccess"));
    } else {
      toast.fromError(t("toolTest.copyNameFailed"));
    }
  };
  ...
  return (
    <div className="space-y-4 h-full">
        ...
        <div className="flex items-center space-x-3">
          {tool.mcp_info.logo_url && (
            ...
            <img
              ...
              alt={t("toolTest.logoAlt", { name: tool.mcp_info.server_name })}
              className="w-6 h-6 object-contain"
            />
          )}
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2 mb-1">
              <h2 className="text-lg font-semibold text-foreground">{t("toolTest.testTool")}</h2>
              <div
                ...
                title={t("toolTest.copyNameTooltip")}
              >
            ...
            <p className="text-xs text-muted-foreground">
              {t("toolTest.provider", { name: tool.mcp_info.server_name })}
            </p>
          </div>
        </div>
        <Button
      ...
      <div className="grid grid-cols-2 gap-4 h-full">
        ...
        <div className="bg-card border border-border rounded-lg">
          <div className="border-b border-border px-4 py-2">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-foreground">{t("toolTest.inputParameters")}</h3>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger
                    render={<CircleHelp className="size-4 cursor-help text-muted-foreground hover:text-foreground" />}
                  />
                  <TooltipContent>{t("toolTest.inputParametersTooltip")}</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          </div>
        ...
        <div className="bg-card border border-border rounded-lg">
          <div className="border-b border-border px-4 py-2">
            <h3 className="text-sm font-semibold text-foreground">{t("toolTest.toolResult")}</h3>
          </div>
            ...
            {!result && !error && !isLoading ? (
              ...
              <div className="flex flex-col justify-center items-center h-48 text-muted-foreground">
                <div className="text-center max-w-sm">
                  ...
                  <h4 className="text-sm font-medium text-foreground mb-1">{t("toolTest.readyToCall")}</h4>
                  <p className="text-xs text-muted-foreground leading-relaxed">{t("toolTest.readyHint")}</p>
                </div>
              </div>
            ) : (
                  ...
                  <div className="p-2 bg-success/10 border border-success/20 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <h4 className="text-xs font-medium text-success">{t("toolTest.executedSuccessfully")}</h4>
                        {duration !== null && (
                ...
                <div className="max-h-96 overflow-y-auto">
                  {isLoading && (
                    <div className="flex flex-col justify-center items-center h-48 text-muted-foreground">
                      ...
                      <p className="text-sm font-medium mt-3">{t("toolTest.callingTool")}</p>
                      <p className="text-xs text-muted-foreground mt-1">{t("toolTest.callingToolHint")}</p>
                    </div>
                  )}
                    ...
                    <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3">
                      <div className="flex items-start space-x-2">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-1">
                            <h4 className="text-xs font-medium text-destructive">{t("toolTest.callFailed")}</h4>
                            {duration !== null && (
                                  ...
                                  <div className="flex items-center space-x-2 p-3 bg-info/10 border border-info/20 rounded-sm">
                                    ...
                                    <div className="flex-1">
                                      <p className="text-xs font-medium text-info">
                                        {t("toolTest.resourceType", { type: content.resource_type })}
                                      </p>
                                      {content.url && (
                                        <a
                                          href={content.url}
