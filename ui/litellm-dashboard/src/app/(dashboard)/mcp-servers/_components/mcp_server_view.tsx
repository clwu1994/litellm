    ...
    <div className="max-w-full p-4">
      <div className="mb-6">
        ...
        <div className="flex items-center gap-2">
          <h1 className="text-2xl font-semibold">{mcpServer.server_name || mcpServer.alias || t("view.unnamed")}</h1>
          <Button
            variant="ghost"
            size="icon-sm"
        ...
        <TabsContent value="2" keepMounted>
          <Card className="p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-medium">{t("view.settings.title")}</h2>
              {editing ? null : (
                <Button variant="outline" onClick={() => setEditing(true)}>
                  {t("view.settings.edit")}
                </Button>
              )}
            </div>
            {editing ? (
              ...
              <div className="divide-y divide-border">
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.serverName")}</p>
                  <div className="col-span-2 text-sm">
                    {mcpServer.server_name || <span className="text-muted-foreground">—</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.alias")}</p>
                  <div className="col-span-2 font-mono text-sm">
                    {mcpServer.alias || <span className="text-muted-foreground">—</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.description")}</p>
                  <div className="col-span-2 text-sm">
                    {mcpServer.description || <span className="text-muted-foreground">—</span>}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.url")}</p>
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.transport")}</p>
                  <div className="col-span-2">
                    {getTransportBadge(handleTransport(mcpServer.transport, mcpServer.spec_path))}
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.authentication")}</p>
                  <div className="col-span-2">{getAuthBadge(handleAuth(mcpServer.auth_type))}</div>
                </div>
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.extraHeaders")}</p>
                  <div className="col-span-2 text-sm">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.allowAllKeys")}</p>
                  <div className="col-span-2">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.networkAccess")}</p>
                  <div className="col-span-2">
                ...
                {handleAuth(mcpServer.auth_type) === "oauth2" && (
                  <div className="grid grid-cols-3 gap-4 py-3">
                    <p className="text-sm font-medium text-muted-foreground">
                      {t("view.fields.delegateAuthToUpstream")}
                    </p>
                    <div className="col-span-2">
                      {mcpServer.delegate_auth_to_upstream ? (
                        <Badge variant="outline">
                          <span className="h-1.5 w-1.5 rounded-full bg-success" />
                          {t("view.enabledPkcePassthrough")}
                        </Badge>
                      ) : (
                        <Badge variant="outline">{t("view.disabled")}</Badge>
                      )}
                ...
                {handleAuth(mcpServer.auth_type) !== "oauth2" &&
                  ...
                  mcpServer.extra_headers.some((h) => typeof h === "string" && h.toLowerCase() === "authorization") && (
                    <div className="grid grid-cols-3 gap-4 py-3">
                      <p className="text-sm font-medium text-muted-foreground">{t("view.fields.oauthPassthrough")}</p>
                      <div className="col-span-2">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.accessGroups")}</p>
                  <div className="col-span-2">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.allowedTools")}</p>
                  <div className="col-span-2">
                ...
                <div className="grid grid-cols-3 gap-4 py-3">
                  <p className="text-sm font-medium text-muted-foreground">{t("view.fields.cost")}</p>
                  <div className="col-span-2">
                    <MCPServerCostDisplay costConfig={mcpServer.mcp_info?.mcp_server_cost_info} />
                  </div>
                </div>
