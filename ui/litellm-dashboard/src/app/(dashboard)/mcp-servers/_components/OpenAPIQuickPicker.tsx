...
const OpenAPIQuickPicker: React.FC<OpenAPIQuickPickerProps> = ({ accessToken, selectedName, onSelect }) => {
  ...
  if (loading) {
    return (
      <div className="mb-4">
        <span className="text-sm font-medium">{t("form.openapiQuickPicker.popularApis")}</span>
  ...
  return (
    <div className="mb-4">
      <span className="mb-2 block text-sm font-medium">{t("form.openapiQuickPicker.popularApis")}</span>
      ...
      <p className="mt-2 text-xs text-muted-foreground">{t("form.openapiQuickPicker.hint")}</p>
    </div>
  );
};
