...
const buildSchema = (required: readonly MCPUserEnvVarSpec[], t: TFunction<"mcpServers">) =>
  z.object(
    Object.fromEntries(
      required.map((spec) => [
        spec.name,
        spec.is_set ? z.string() : z.string().min(1, t("userEnvVars.fieldRequired", { name: spec.name })),
      ]),
    ),
  );
          ...
          <FormField
            ...
            label={
              <span className="flex items-center gap-2">
                ...
                {spec.is_set && <Badge variant="secondary">{t("userEnvVars.set")}</Badge>}
              </span>
            }
          >
            {(field) => (
              <PasswordInput
                ...
                placeholder={
                  spec.is_set
                    ...
                    : spec.description || t("userEnvVars.enterYour", { name: spec.name })
                }
              />
            )}
          </FormField>
...
const UserEnvVarsModal: React.FC<UserEnvVarsModalProps> = ({ server, open, accessToken, onClose, onSaved }) => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (saved) => {
      toast.success(t("userEnvVars.saved"));
      onSaved?.(saved);
      onClose();
    },
    onError: (err) => {
      toast.fromError(t("userEnvVars.saveFailed", { message: err instanceof Error ? err.message : String(err) }));
    },
  });
  ...
  const displayName = server?.server_name || server?.alias || server?.server_id || t("userEnvVars.fallbackName");
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[520px]">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <StatusBadge tone="info" label={t("form.envVars.scopePerUser")} />
          </div>
        ...
        <div className="mt-2 space-y-4">
          {isLoading ? (
            ...
            <>
              <span className="block text-sm text-muted-foreground">{t("userEnvVars.intro")}</span>
