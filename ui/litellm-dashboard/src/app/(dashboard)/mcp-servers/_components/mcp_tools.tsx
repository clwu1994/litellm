        ...
        <div className="grid h-auto w-full grid-cols-4 gap-4">
            ...
            <div className="flex flex-col flex-1">
                ...
                <div className="mb-4 rounded-lg border border-border bg-card p-3">
                  <div className="mb-2 flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <p className="text-sm font-medium">{t("tools.additionalHeaders")}</p>
                    </div>
                    <Button variant="link" size="sm" onClick={() => setShowHeaderInput(!showHeaderInput)}>
                      {showHeaderInput ? t("tools.hide") : t("tools.configure")}
                    </Button>
                  </div>

                  {!showHeaderInput && Object.keys(passthroughHeaders).length === 0 && (
                    <p className="text-xs text-muted-foreground">{t("tools.headersRequired")}</p>
                  )}
                      ...
                      {extraHeaders?.map((headerName) => (
                        <div key={headerName}>
                          ...
                          <InputGroup className="w-full">
                            ...
                            <InputGroupInput
                              placeholder={t("tools.enterHeader", { header: headerName })}
                  ...
                  {!showHeaderInput && Object.keys(passthroughHeaders).length > 0 && (
                    <div className="mt-2">
                      <p className="flex items-center text-xs text-muted-foreground">
                        ...
                        {t("tools.headersConfigured", { count: Object.keys(passthroughHeaders).length })}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col flex-1 min-h-0">
                <p className="mb-3 flex items-center text-sm font-medium">
                  <Wrench className="mr-2 size-4" /> {t("tools.availableTools")}
                  {toolsData.length > 0 && (
                ...
                {usesBrowserHeldToken && !oauthToken && (
                  <div className="rounded-lg border border-border bg-card p-4 text-center">
                    ...
                    <p className="mb-1 text-xs font-medium">{t("tools.authRequired")}</p>
                    <p className="mb-3 text-xs text-muted-foreground">{t("tools.authenticateToView")}</p>
                    <Button
                      size="sm"
                ...
                {(authorizationCodeNeedsAuth || authorizationCodeTokenRejected) && (
                  <div className="rounded-lg border border-border bg-card p-4 text-center">
                    <Lock className="mx-auto mb-2 size-6 text-muted-foreground" />
                    <p className="mb-1 text-xs font-medium">{t("tools.authRequired")}</p>
                    <p className="mb-3 text-xs text-muted-foreground">{t("tools.authenticateUpstream")}</p>
                    <Button
                      size="sm"
                  ...
                  <>
                    ...
                    {toolsAreaLoading && (
                      <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card py-8">
                        ...
                        <p className="text-xs font-medium">{t("tools.loading")}</p>
                      </div>
                    )}
                    ...
                    {!toolsAreaLoading &&
                      ...
                      (!toolsData || toolsData.length === 0) && (
                        <div className="rounded-lg border border-border bg-card p-4 text-center">
                          ...
                          <p className="mb-1 text-xs font-medium">{t("tools.noToolsAvailable")}</p>
                          <p className="text-xs text-muted-foreground">{t("tools.noToolsForServer")}</p>
                        </div>
                      )}
                      ...
                      <>
                        {filteredTools.length === 0 ? (
                          <div className="rounded-lg border border-border bg-card p-4 text-center">
                            ...
                            <p className="mb-1 text-xs font-medium">{t("tools.noToolsFound")}</p>
                            <p className="text-xs text-muted-foreground">
                              {t("tools.noMatch", { term: toolSearchTerm })}
                            </p>
                          </div>
                        ) : (
                                ...
                                <div className="flex items-start space-x-2">
                                  {tool.mcp_info.logo_url && (
                                    <img
                                      ...
                                      alt={t("tools.logoAlt", { name: tool.mcp_info.server_name })}
          ...
          <div className="col-span-3 flex flex-col">
            <div className="flex items-center justify-between border-b border-border p-4">
              <h2 className="mb-0 text-xl font-semibold">{t("tools.playgroundTitle")}</h2>
            </div>
            ...
            <div className="flex-1 overflow-auto p-4">
              {!selectedTool ? (
                ...
                <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
                  ...
                  <p className="mb-2 text-lg font-medium">{t("tools.selectTool")}</p>
                  <p className="max-w-md text-center text-sm">{t("tools.selectToolHint")}</p>
                </div>
              ) : (
                /* Tool Test Panel */
