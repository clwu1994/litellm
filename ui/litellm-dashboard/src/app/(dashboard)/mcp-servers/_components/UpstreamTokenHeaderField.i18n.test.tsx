...
describe("UpstreamTokenHeaderField Chinese copy", () => {
  ...
  it("renders the Chinese tooltip in the same open state and hides the English original", async () => {
    ...
    await expectTooltipPair(
      user,
      "Token 请求头（可选）",
      "LiteLLM 为此服务器解析的 Token 由哪个上游请求头承载。留空则按 'Authorization: Bearer <token>' 发送，这是默认值，也是大多数服务器期望的方式。当上游期望在其他位置接收时，请设置请求头名称；例如某个 API 网关在 'esb-oauth' 上终止自己的凭证，而来自 Static Headers 的独立 Authorization 会透传到其后的服务器。",
