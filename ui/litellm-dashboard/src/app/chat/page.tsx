...
const SUGGESTIONS = [
  { id: "write", labelKey: "page.suggestionWrite" },
  { id: "learn", labelKey: "page.suggestionLearn" },
  { id: "code", labelKey: "page.suggestionCode" },
  { id: "brainstorm", labelKey: "page.suggestionBrainstorm" },
] as const;
const LOCALSTORAGE_MODEL_KEY = "litellm_chat_selected_model";

function getGreeting(t: TFunction<"chat">): string {
  const h = new Date().getHours();
  if (h >= 5 && h < 12) return t("page.goodMorning");
  if (h >= 12 && h < 17) return t("page.goodAfternoon");
  return t("page.goodEvening");
}
...
export default function ChatConversationPage() {
  ...
  useEffect(() => {
    ...
    fetchAvailableModels(accessToken)
      ...
      .catch(() => toast.error(t("page.toastModelsFailed")))
    ...
    async (text: string, historyOverride?: Array<{ role: "user" | "assistant"; content: string }>) => {
      ...
      try {
        ...
        if (err instanceof Error && err.name === "AbortError") {
          ...
          updateLastAssistantMessage(convId!, {
            content: t("page.partialResponseSaved"),
          });
        }
      } finally {
  ...
  const greeting = displayName
    ? t("page.greetingWithName", { greeting: getGreeting(t), name: displayName })
    : getGreeting(t);
    ...
    <div className="w-[280px] h-[400px] flex flex-col overflow-hidden">
      <div className="p-2 pb-1">
        <Input
          ...
          placeholder={t("page.searchModels")}
        ...
        render={
          <Button variant="outline" size="sm" className="max-w-[240px] justify-start gap-1.5 overflow-hidden">
            {selectedModel ? (
              ...
              <span className="text-muted-foreground">{t("page.selectModel")}</span>
            )}
          ...
          {inConversation && selectedMCPServers.length > 0 && (
            <span className="text-xs text-muted-foreground max-w-[160px] overflow-hidden text-ellipsis whitespace-nowrap">
              {selectedMCPServers.length === 1
                ? t("page.toolsConnectedOne", { total: selectedMCPServers.length })
                : t("page.toolsConnectedOther", { total: selectedMCPServers.length })}
            </span>
          )}
          {isStreaming ? (
    ...
    <>
      {storageUnavailable && !storageBannerDismissed && (
        <div className="bg-warning/10 border-b border-warning/20 px-5 py-1.5 text-[13px] text-warning flex justify-between items-center">
          <span>{t("page.storageBanner")}</span>
          <Button
            variant="ghost"
            size="icon-xs"
          ...
          <div className="flex-1 min-h-0 flex flex-col mx-auto w-full px-6 relative" style={{ maxWidth: 760 }}>
            ...
            {showScrollButton && (
              <Button
                ...
                aria-label={t("page.scrollToBottom")}
              >
